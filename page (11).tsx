...
import {
  ComposerStepRail,
  type ComposerStep,
} from "../../components/composer-step-rail";
...
import {
  channelCanAutoPublish,
  channelCanManualPublish,
  channelIsReady,
  channelStatusLabel,
  findChannelAccount,
  loadChannelAccounts,
...
function ComposePageContent() {
  ...
  const [editingImageSource, setEditingImageSource] =
    useState<ComposerImageEditSource | null>(null);
  ...
  const [previewChannel, setPreviewChannel] =
    useState<PublishingChannel>("rubika");
  const [instagramPreviewMode, setInstagramPreviewMode] = useState<
    "feed" | "story" | "reel"
  >("feed");
  ...
  const selectedMedia = useMemo(() => {
    ...
    return (
      mediaAssets.find((asset) => String(asset.id) === selectedMediaId) ?? null
    );
  }, [mediaAssets, selectedMediaId]);

  const previewImageUrl =
    selectedFilePreviewUrl ||
    (selectedMedia ? mediaPreviewUrls[selectedMedia.id] : "");
  const brandAvatarUrl = store?.avatar_asset_id
    ? mediaPreviewUrls[store.avatar_asset_id]
    : store?.logo_asset_id
      ? mediaPreviewUrls[store.logo_asset_id]
      : "";
  const selectedCampaign = useMemo(() => {
    if (form.campaign_id === null) return null;
    return (
      campaigns.find((campaign) => campaign.id === form.campaign_id) ?? null
    );
  }, [campaigns, form.campaign_id]);

  const finalPreview = useMemo(() => {
    const hasVisiblePostContent = Boolean(
      form.caption.trim() || previewImageUrl,
    );
    return [
      form.caption,
      hasVisiblePostContent ? store?.default_cta : "",
      hasVisiblePostContent ? store?.caption_footer : "",
      form.hashtags,
    ]
      .filter(Boolean)
      .join("\n\n");
  ...
  const captionSuggestions = useMemo(() => {
    ...
    const numberMatches = form.caption.match(
      /(?:عدد|کلمه)\s*([0-9\u06f0-\u06f9]+)/i,
    );
  ...
  const selectedChannels = useMemo(
    () => normalizeChannels(form.platform),
    [form.platform],
  );
  ...
  const channelNotes = selectedChannels.map((channel) => {
    ...
    if (!account)
      return `${channel === "rubika" ? "روبیکا" : "اینستاگرام"} هنوز در مرکز کانال‌ها ثبت نشده است.`;
    if (channelCanAutoPublish(account))
      return `${channel === "rubika" ? "روبیکا" : "اینستاگرام"} برای انتشار خودکار آماده است.`;
    if (channelCanManualPublish(account))
      return `${channel === "rubika" ? "روبیکا" : "اینستاگرام"} در حالت انتشار دستی/یادآوری آماده است.`;
  ...
  const instagramAutomationValid =
    !autoReplyEnabled ||
    (Boolean(triggerKeywords.trim()) && Boolean(privateReplyMessage.trim()));

  const readinessItems = [
    {
      label: "عنوان داخلی",
      detail: hasTitle
        ? "عنوان برای مدیریت محتوا ثبت شده است."
        : "برای ذخیره پست، عنوان داخلی لازم است.",
      done: hasTitle,
      required: true,
    },
    {
      label: "متن یا رسانه",
      detail: hasPostBody
        ? "پست محتوای قابل انتشار دارد."
        : "حداقل کپشن یا تصویر برای آماده‌سازی پیشنهاد می‌شود.",
      done: hasPostBody,
      required: true,
    },
    {
      label: "طول متن روبیکا",
      detail: rubikaLengthValid
        ? "طول متن در محدوده مجاز روبیکا است (کمتر از ۴۰۰۰ حرف)."
        : "متن برای روبیکا بسیار طولانی است (باید کمتر از ۴۰۰۰ حرف باشد).",
      done: rubikaLengthValid,
      required: rubikaSelected,
    },
    {
      label: "طول متن اینستاگرام",
      detail: instagramLengthValid
        ? "طول متن در محدوده مجاز اینستاگرام است (کمتر از ۲۲۰۰ حرف)."
        : "متن برای اینستاگرام بسیار طولانی است (باید کمتر از ۲۲۰۰ حرف باشد).",
      done: instagramLengthValid,
      required: instagramSelected,
    },
    {
      label: "رسانه اینستاگرام",
      detail: instagramMediaValid
        ? "تصویر برای پست اینستاگرام انتخاب شده است."
        : "پست اینستاگرام نیاز به حداقل یک تصویر دارد.",
      done: instagramMediaValid,
      required: instagramSelected,
    },
    {
      label: "تنظیمات تعامل خودکار",
      detail: instagramAutomationValid
        ? "تنظیمات کلمات کلیدی و پاسخ دایرکت معتبر است."
        : "در صورت فعال بودن تعامل خودکار، کلمات کلیدی و پاسخ دایرکت الزامی است.",
    ...
    },
    {
      label: "کانال روبیکا",
      detail: rubikaReady
        ? "روبیکا آماده انتشار خودکار است."
        : `روبیکا: ${channelStatusLabel(rubikaChannel)}`,
    ...
    },
    {
      label: "کانال انتشار",
      detail:
        instagramSelected && instagramManualReady
          ? "اینستاگرام در حالت انتشار دستی/یادآوری آماده است."
          : instagramSelected && instagramReady
            ? "اینستاگرام برای مسیر انتخاب‌شده آماده است."
            : instagramSelected && rubikaReady
              ? "روبیکا آماده است؛ اینستاگرام هنوز نیازمند اقدام کانالی است."
              : instagramSelected
                ? `اینستاگرام: ${channelStatusLabel(instagramChannel)}`
                : "کانال انتخاب‌شده برای worker فعال است.",
      done: selectedChannels.every((channel) =>
        channelIsReady(findChannelAccount(channelAccounts, channel)),
      ),
    ...
    },
    {
      label: "زمان انتشار",
      detail: hasSchedule
        ? "پست می‌تواند وارد صف زمان‌بندی شود."
        : "بدون زمان انتشار، پست به عنوان پیش‌نویس یا آماده ذخیره می‌شود.",
      done: hasSchedule,
    },
    {
      label: "کیت برند",
      detail:
        store?.brand_voice || store?.default_cta
          ? "لحن یا CTA برند برای خروجی آماده است."
          : "در تنظیمات فروشگاه، لحن و CTA برند را کامل کنید.",
  ...
  const publishTone = canSchedule
    ? "success"
    : canMarkReady
      ? "primary"
      : "warning";
  const publishStateLabel = canSchedule
    ? "آماده زمان‌بندی"
    : canMarkReady
      ? "آماده بازبینی"
      : "در حال تولید";
  ...
  const scheduleLabel = form.scheduled_at
    ? new Date(form.scheduled_at).toLocaleString("fa-IR", {
        dateStyle: "medium",
        hour: "2-digit",
        minute: "2-digit",
        timeZone: scheduleTimezone,
  ...
  const workspaceModes: Array<{
  ...
  }> = [
    {
      ...
      value: "content",
      icon: FileText,
      done: hasTitle && hasPostBody,
      detail: `${captionLength} کاراکتر`,
    },
    {
      label: "رسانه",
      value: "media",
      ...
      detail: previewImageUrl ? "تصویر انتخاب شد" : "اختیاری",
    },
    {
      label: "انتشار",
      value: "workflow",
      icon: LayoutTemplate,
        ...
        form.campaign_id || form.scheduled_at || selectedChannels.length,
      ),
      detail: `${selectedChannelLabel} · ${campaignLabel}`,
    },
  ];
  const workflowCards = [
    {
      ...
      detail: hasReadyPublishingChannel ? "آماده انتشار" : "نیازمند تکمیل",
      tone: hasReadyPublishingChannel ? "success" : "warning",
      icon: AtSign,
    },
    {
      label: "کمپین",
      value: campaignLabel,
      detail: selectedCampaign?.goal || "برای گزارش‌گیری قابل اتصال است",
      tone: selectedCampaign ? "primary" : "neutral",
      icon: Megaphone,
    },
    {
      label: "زمان",
      value: scheduleLabel,
      detail: hasSchedule ? "وارد صف می‌شود" : "پیش‌نویس باقی می‌ماند",
  ...
  const composerSteps: ComposerStep[] = [
    {
      label: "محتوا",
      helper:
        hasTitle && hasPostBody
          ? "عنوان و محتوای اصلی آماده است."
          : "عنوان داخلی و کپشن یا رسانه را کامل کنید.",
      icon: FileText,
      state:
        activeStepIndex === 0
          ? "active"
    ...
    {
      label: "رسانه",
      helper: previewImageUrl
        ? "تصویر خروجی انتخاب شده است."
        : "رسانه اختیاری است؛ برای پست تصویری انتخاب کنید.",
      icon: Images,
      state:
        activeStepIndex === 1 ? "active" : previewImageUrl ? "done" : "pending",
    },
    {
      label: "زمان انتشار",
      helper: hasSchedule
        ? "تاریخ و ساعت ورود به صف مشخص است."
        : "برای انتشار خودکار، تاریخ و ساعت را انتخاب کنید.",
      icon: CalendarClock,
      state:
        activeStepIndex === 2 ? "active" : hasSchedule ? "done" : "pending",
    },
    {
      label: "بازبینی نهایی",
      helper: !hasReadyPublishingChannel
        ? "حداقل یک کانال آماده برای زمان‌بندی لازم است."
        : instagramSelected && !instagramReady
          ? "اینستاگرام هنوز نیازمند اقدام است؛ کانال آماده دیگر می‌تواند ادامه دهد."
          : reviewBlocksSchedule
            ? "این پست قبل از زمان‌بندی باید تایید شود."
            : canSchedule
              ? "پست آماده ورود به صف انتشار است."
              : "پیش‌نمایش و الزام‌های انتشار را بررسی کنید.",
      icon: ShieldCheck,
      state:
        activeStepIndex === 3 ? "active" : canSchedule ? "done" : "pending",
    },
  ];
  const studioPanels: Array<{
    label: string;
    value: StudioPanel;
    icon: typeof Eye;
    ready?: boolean;
  ...
  const loadData = useCallback(async () => {
    ...
    const [
      ...
      editingPostId
        ? fetch(`${apiUrl}/posts/${editingPostId}`, { headers })
    ...
    if (editingPostId) {
      ...
      setShowOptionalDetails(
        Boolean(post.campaign_id || post.campaign || post.internal_note),
      );

      const attachedAsset = loadedMediaAssets.find(
        ...
        if (rulesResponse.ok) {
          ...
          const loadedTemplates =
            rulesData.rules?.filter(
              (r: InstagramAutomationRule) => r.is_template,
            ) ?? [];
          setTemplates(loadedTemplates);

          const existingRule =
            rulesData.rules?.find(
              (r: InstagramAutomationRule) =>
                String(r.post_id) === editingPostId,
            ) ?? null;
          if (existingRule) {
    ...
    loadData().catch((err) => {
      setError(
        err instanceof Error
          ? err.message
          : isEditing
            ? "خطا در دریافت اطلاعات پست برای ویرایش"
            : "خطا در دریافت اطلاعات اولیه composer",
      );
      setLoading(false);
    });
  }, [isEditing, loadData]);
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      window.localStorage.setItem(
        localDraftKey,
        JSON.stringify({ form, selectedMediaId, savedAt }),
      );
  ...
  function updateField(
    ...
    value: (typeof emptyForm)[keyof typeof emptyForm],
  ) {
  ...
  async function quickCreateCampaign() {
    ...
    if (!name) {
      showToast({
        ...
        description: "برای ساخت کمپین، یک نام کوتاه وارد کنید.",
        tone: "warning",
      });
      return;
    }
    ...
    try {
      const createdCampaign = await createCampaign({
        name,
        color:
          store?.brand_accent_color || store?.brand_primary_color || "#0F766E",
      });
      setCampaigns((current) => [
        createdCampaign,
        ...current.filter((campaign) => campaign.id !== createdCampaign.id),
      ]);
      setForm((current) => ({
        ...current,
        campaign_id: createdCampaign.id,
        campaign: createdCampaign.name,
      }));
      setQuickCampaignName("");
      showToast({
        title: "کمپین ساخته شد",
        description: "پست فعلی به کمپین جدید متصل شد.",
        tone: "success",
      });
    } catch (err) {
      const nextError =
        err instanceof Error ? err.message : "ساخت کمپین ناموفق بود";
      setError(nextError);
      showToast({
        title: "ساخت کمپین ناموفق بود",
  ...
  function applyDefaults() {
    const defaultCaption = [store?.description, store?.default_cta]
      .filter(Boolean)
      .join("\n\n");
    setForm((current) => ({
      ...current,
  ...
  function startWithDefaults() {
    ...
    window.setTimeout(
      () =>
        document
          .getElementById("composer-workspace")
  ...
  function resetComposer(
    options: { clearStatus?: boolean } = { clearStatus: true },
  ) {
    setActiveStepIndex(0);
    setWorkspaceMode("content");
    setStudioPanel("preview");
    ...
    if (editingPost) {
      ...
      setShowOptionalDetails(
        Boolean(
          ...
          editingPost.internal_note,
        ),
      );
      const attachedAsset = mediaAssets.find(
        (asset) => asset.post_id === editingPost.id,
      );
      setSelectedMediaId(attachedAsset ? String(attachedAsset.id) : "");
    } else {
  ...
  async function saveEditedComposerImage(file: File) {
    ...
    try {
      ...
      formData.append(
        "tags",
        [editingImageSource.tags, "edited", "composer"]
          .filter(Boolean)
          .join(", "),
      );
      const response = await fetch(`${apiUrl}/media`, {
      ...
      showToast({
        title: "تصویر ویرایش‌شده انتخاب شد",
        description: savedAsset.original_filename,
        tone: "success",
      });
    } catch (err) {
      const nextError =
        err instanceof Error ? err.message : "خطای ذخیره نسخه ویرایش‌شده";
      setError(nextError);
      showToast({
        title: "ذخیره ویرایش تصویر ناموفق بود",
        description: nextError,
        tone: "alert",
      });
    } finally {
  ...
  async function syncSelectedMedia(postId: number) {
    const attachedAssets = mediaAssets.filter(
      (asset) => asset.post_id === postId,
    );
    const uploadedAsset = await uploadSelectedFile();

    if (uploadedAsset) {
      await Promise.all(
        attachedAssets.map((asset) => attachMedia(asset.id, null)),
      );
    ...
    if (selectedMediaId) {
      await Promise.all(
        attachedAssets
          .filter((asset) => String(asset.id) !== selectedMediaId)
          .map((asset) => attachMedia(asset.id, null)),
      );
  ...
  async function persistPost(action: SaveAction) {
    if (!canSaveDraft) {
      ...
      showToast({
        title: "عنوان داخلی لازم است",
        description: "قبل از ذخیره، یک عنوان برای مدیریت محتوا وارد کنید.",
        tone: "warning",
      });
      return;
    }
    if (action === "ready" && !canMarkReady) {
      ...
      showToast({
        title: "محتوای پست کامل نیست",
        description: "برای آماده‌سازی، کپشن یا تصویر اضافه کنید.",
        tone: "warning",
      });
      return;
    }
    if (action === "schedule" && !canSchedule) {
      ...
      showToast({
        title: "زمان‌بندی هنوز آماده نیست",
    ...
    try {
      ...
      if (instagramSelected && autoReplyEnabled) {
        ...
        if (keywordsArray.length === 0) {
          throw new Error(
            "برای فعال‌سازی تعامل خودکار، حداقل یک کلیدواژه لازم است.",
          );
        }
        if (!privateReplyMessage.trim()) {
          throw new Error(
            "برای فعال‌سازی تعامل خودکار، متن پاسخ دایرکت لازم است.",
          );
        }
        ...
        if (!ruleResponse.ok) {
          ...
          throw new Error(
            errData.detail || "ذخیره قانون تعامل خودکار اینستاگرام ناموفق بود",
          );
        }
      ...
      showToast({
        ...
        description:
          action === "schedule"
            ? "پست در صف انتشار قرار گرفت."
            : "نسخه جدید در فضای کاری ثبت شد.",
        tone: "success",
      });
      await loadData();
    } catch (err) {
      ...
      <AppShell>
        <WorkspacePage className="space-y-3 sm:space-y-4">
            ...
            >
              <div className="space-y-4">
                ...
                <div className="grid gap-2">
                  <label className="text-xs font-black text-app-text">
                    ساخت کمپین جدید
                  </label>
                  <div className="flex gap-2">
                    <Input
                      value={quickCampaignName}
                      onChange={(event) =>
                        setQuickCampaignName(event.target.value)
                      }
                ...
                <div className="rounded-md bg-app-surfaceMuted/85 p-2.5 shadow-hairline mt-4">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`h-4 w-4 text-slate-500 transition ${showOptionalDetails ? "rotate-180" : ""}`}
                      aria-hidden="true"
                    />
                  </button>
                  {showOptionalDetails ? (
                    <Textarea
                      ...
                      onChange={(event) =>
                        updateField("internal_note", event.target.value)
                      }
          ...
          <section className="nahrino-card relative overflow-hidden rounded-lg px-3 py-2.5 sm:px-4 sm:py-3">
            <div
              className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_20%,rgba(20,184,166,0.12),transparent_30%),radial-gradient(circle_at_78%_10%,rgba(59,130,246,0.10),transparent_28%)]"
              aria-hidden="true"
            />
            <div className="relative">
              <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
                <div>
                  <p className="text-[10px] font-black text-app-primary">
                    ساخت محتوا
                  </p>
                  <h1 className="mt-1 text-xl font-black text-app-text">
                    {isEditing ? "ویرایش پست" : "پست جدید"}
                  </h1>
                  <p className="mt-1 max-w-2xl text-xs leading-5 text-app-muted">
                    یک مسیر حرفه‌ای برای ساخت، پیش‌نمایش، زمان‌بندی و انتشار
                    چندکاناله؛ هر کنترل فقط در مرحله مرتبط دیده می‌شود.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {hasReadyPublishingChannel
                      ? `${selectedReadyChannels.length} کانال آماده`
                      : "تکمیل کانال لازم است"}
                  </StatusToken>
                  {!isEditing ? (
                    <StatusToken
                      tone={autosaveState === "dirty" ? "warning" : "neutral"}
                    >
                  ...
                  {editingPost ? (
                    <ApprovalBadge
                      status={editingPost.approval_status}
                      compact
                    />
                  ) : null}
                  <Button
          ...
          <form
            onSubmit={saveDraft}
            className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_380px]"
          >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-5 text-app-muted">
                      وضعیت‌های مهم بدون باز کردن فرم‌های اضافه.
                    </p>
                  </div>
                  <StatusToken tone={publishTone}>
                    {readinessScore}%
                  </StatusToken>
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          <span className="block text-[11px] font-black text-app-muted">
                            {card.label}
                          </span>
                        ...
                        <StatusToken tone={card.tone} size="sm">
                          {card.tone === "success"
                            ? "آماده"
                            : card.tone === "primary"
                              ? "متصل"
              ...
              <WorkspacePanel
                ...
                action={
                  <div className="flex flex-wrap items-center gap-2">
                    <ChannelBadges platform={form.platform} compact />
                    {hasSchedule ? (
                      <Tag tone="success">زمان‌بندی شده</Tag>
                    ) : null}
                  </div>
                }
                bodyClassName="p-0"
              >
                      ...
                      <button
                        ...
                        className={`app-interactive flex min-w-0 items-center gap-2 rounded-md px-2.5 py-2 text-right transition ${
                          active
                            ...
                            : "text-app-muted hover:bg-white/75 hover:text-app-text"
                        }`}
                      >
                        <span
                          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md ${active ? "bg-app-soft text-app-primary" : "bg-white text-slate-500 shadow-hairline"}`}
                        >
                          <Icon className="h-4 w-4" aria-hidden="true" />
                        </span>
                        <span className="min-w-0">
                          ...
                          <span className="mt-0.5 block truncate text-[10px] text-app-muted">
                            {mode.detail}
                          </span>
                        </span>
                      </button>
                ...
                <div className="p-3 sm:p-4">
                  {workspaceMode === "content" ? (
                    <div
                      id="composer-content"
                      className={`grid gap-3 lg:grid ${activeStepIndex === 0 ? "grid" : "hidden"}`}
                    >
                      <div className="rounded-md border border-white/70 bg-white/75 p-3 shadow-hairline backdrop-blur-xl">
                        <div className="mb-3 flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
                          <div>
                            ...
                            <p className="mt-1 text-xs leading-5 text-app-muted">
                              عنوان برای تیم است؛ کپشن و هشتگ‌ها وارد خروجی
                              مخاطب می‌شوند.
                            </p>
                          </div>
                          <Button
                        ...
                        <div className="grid gap-3">
                          <Field
                            label="عنوان داخلی پست"
                            required
                            hint="فقط برای مدیریت محتوا و صف انتشار؛ مخاطب این عنوان را نمی‌بیند."
                          >
                            <Input
                              value={form.title}
                              onChange={(event) =>
                      ...
                      <div className="grid gap-2 sm:grid-cols-3">
                        <button
                          ...
                          className="app-interactive rounded-md border border-app-border bg-white/70 p-3 text-right shadow-hairline hover:bg-white"
                        >
                          <Images
                            className="mb-2 h-4 w-4 text-app-primary"
                            aria-hidden="true"
                          />
                  ...
                  {workspaceMode === "media" ? (
                    <div
                      id="composer-media"
                      className={`grid gap-3 lg:grid-cols-[220px_minmax(0,1fr)] ${activeStepIndex === 1 ? "grid" : "hidden"}`}
                    >
                      <div className="space-y-3">
                        ...
                        {previewImageUrl ? (
                          ...
                          <div className="rounded-md bg-app-surfaceMuted p-3 text-xs leading-5 text-app-muted shadow-hairline">
                            تصویر اختیاری است، اما برای خروجی حرفه‌ای بهتر است
                            یک رسانه انتخاب شود.
                          </div>
                        )}
                      </div>
                      ...
                      <section className="rounded-md border border-app-border bg-white/72 p-3 shadow-hairline backdrop-blur">
                        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                          <div>
                            ...
                            <p className="mt-1 text-xs leading-5 text-app-muted">
                              برای هر کانال، محدودیت انتشار و آمادگی را قبل از
                              زمان‌بندی ببینید.
                            </p>
                          </div>
                        ...
                        <div className="mt-3 grid gap-2 sm:grid-cols-2">
                          {channelOptions.map((option) => {
                            ...
                            const active = selectedChannels.includes(
                              option.value,
                            );
                            return (
                              <button
                                ...
                                className={`app-interactive rounded-md border px-3 py-3 text-right ${
                                  active
                                    ? "border-blue-200 bg-white text-app-primary shadow-soft"
                                    : "border-app-border bg-white/70 text-app-text hover:bg-white"
                                }`}
                                aria-pressed={active}
                              >
                        ...
                        {channelNotes.length ? (
                          <div className="mt-3 space-y-1.5">
                            {channelNotes.map((note) => (
                              <p
                                key={note}
                                className="text-xs leading-5 text-app-muted"
                              >
                                {note}
                              </p>
                            ))}
                      ...
                      <section className="hidden lg:block rounded-md border border-app-border bg-white/72 p-3 shadow-hairline backdrop-blur">
                        <div className="mb-3 flex items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-xs leading-5 text-app-muted">
                              کمپین فقط برای دسته‌بندی، گزارش و برنامه‌ریزی است؛
                              فرم بزرگ جداگانه ندارد.
                            </p>
                          </div>
                          {selectedCampaign ? (
                            <span
                              className="mt-0.5 h-4 w-4 shrink-0 rounded shadow-hairline"
                              style={{
                          ...
                          <div className="grid gap-2">
                            ...
                            <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
                              <Input
                                ...
                                placeholder="نام کمپین جدید، مثلاً لانچ خرداد"
                              />
                              <Button
                                type="button"
                                variant="secondary"
                            ...
                            {selectedCampaign ? (
                              <p className="line-clamp-2 text-xs leading-5 text-app-muted">
                                {selectedCampaign.goal ||
                                  selectedCampaign.notes ||
                                  "هدف کمپین هنوز تعریف نشده است."}
                              </p>
                              ...
                              <span className="flex min-w-0 items-center gap-2">
                                ...
                                <span>
                                  ...
                                  <span className="mt-1 block text-xs text-app-muted">
                                    {form.internal_note
                                      ? "یادداشت ثبت شده است."
                                      : "برای تیم و تاییدها اختیاری است."}
                                  </span>
                                </span>
                              </span>
                              <ChevronDown
                      ...
                      {instagramSelected ? (
                        <section className="rounded-md border border-app-border bg-white/72 p-3 shadow-hairline backdrop-blur space-y-3">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-sm font-black text-app-text">
                                تعامل خودکار اینستاگرام (کامنت به دایرکت)
                              </p>
                              <p className="mt-1 text-xs leading-5 text-app-muted">
                                ارسال خودکار پیام دایرکت و پاسخ به کامنت در صورت
                                دریافت کلمه کلیدی.
                              </p>
                            </div>
                          ...
                          {autoReplyEnabled ? (
                            <div className="mt-3 grid gap-3 border-t border-app-border pt-3">
                              {templates.length > 0 ? (
                                <div className="grid gap-2">
                                  <label className="text-xs font-black text-app-text">
                                    انتخاب قالب آماده تعامل خودکار
                                  </label>
                                  <select
                              ...
                              <div className="grid gap-3 lg:grid-cols-2">
                                <Field
                                  label="کلمه کلیدی (کلیدواژه‌ها با کاما جدا شوند)"
                                  required
                                  hint="مثلاً: 5, قیمت, راهنمایی"
                                >
                                  <Input
                              ...
                              <Field
                                label="پیام پاسخ خودکار دایرکت (DM)"
                                required
                                hint="این پیام به صورت خصوصی به دایرکت کاربر فرستاده می‌شود."
                              >
                                <Textarea
                                  value={privateReplyMessage}
                                  onChange={(e) =>
                                    setPrivateReplyMessage(e.target.value)
                                  }
                                  className="min-h-16"
                              ...
                              <div className="rounded-md border border-app-border bg-app-surfaceMuted/50 p-2.5 space-y-2">
                                <div className="flex items-center gap-2">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setPublicReplyEnabled(e.target.checked)
                                    }
                                    className="rounded border-gray-300 text-app-primary focus:ring-app-primary h-4 w-4"
                                  />
                                  <label
                                    htmlFor="publicReplyEnabled"
                                    className="text-xs font-black text-app-text cursor-pointer"
                                  >
                                ...
                                {publicReplyEnabled ? (
                                  <Input
                                    ...
                                    onChange={(e) =>
                                      setPublicReplyMessage(e.target.value)
                                    }
                              ...
                              <div className="border-t border-app-border pt-3 space-y-3">
                                <div>
                                  <p className="text-xs font-black text-app-text">
                                    رفتار در صورت پاسخ مشتری (Operator Takeover)
                                  </p>
                                  <p className="mt-0.5 text-[10px] text-app-muted">
                                    زمانی که کاربر به پیام خودکار شما پاسخ دهد،
                                    اتوماسیون متوقف شده و این گفتگو به اپراتور
                                    واگذار می‌شود.
                                  </p>
                                </div>
                                <div className="grid gap-2 sm:grid-cols-2">
                                  <label
                                    className={`app-interactive flex items-start gap-2.5 rounded-md border p-2.5 text-right cursor-pointer transition ${
                                      onCustomerReply === "hand_off"
                                        ? "border-app-primary bg-white shadow-soft"
                                        : "border-app-border bg-white/70"
                                    }`}
                                  >
                                    <input
                                      type="radio"
                                    ...
                                    <span>
                                      <span className="block text-xs font-black text-app-text">
                                        سکوت و واگذاری گفتگو
                                      </span>
                                      <span className="mt-0.5 block text-[10px] text-app-muted">
                                        بدون پیام اضافه، وضعیت گفتگو را به «در
                                        انتظار پاسخ اپراتور» تغییر دهید.
                                      </span>
                                    </span>
                                  </label>
                                  ...
                                  >
                                    ...
                                    <span>
                                      <span className="block text-xs font-black text-app-text">
                                        ارسال پیام انتظار خودکار
                                      </span>
                                      <span className="mt-0.5 block text-[10px] text-app-muted">
                                        یک پیام پیش‌فرض برای مشتری بفرستید و سپس
                                        گفتگو را به اپراتور واگذار کنید.
                                      </span>
                                    </span>
                                  </label>
                                </div>
                                {onCustomerReply === "send_waiting_message" ? (
                                  <Field
                                    label="متن پیام انتظار خودکار"
                                    required
                                    hint="این پیام قبل از توقف اتوماسیون به دایرکت کاربر فرستاده می‌شود."
                                  >
                                    <Input
                                      value={waitingReplyMessage}
                                      onChange={(e) =>
                                        setWaitingReplyMessage(e.target.value)
                                      }
                  ...
                  {studioPanels.map((panel) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          className={`absolute left-2 top-2 h-1.5 w-1.5 rounded-full ${panel.ready ? "bg-emerald-500" : "bg-slate-300"}`}
                        />
                      </button>
                    );
                  })}
                      ...
                      <div className="mb-3 flex flex-col gap-2.5">
                          ...
                          <div className="flex gap-1">
                            <button
                              ...
                              className={`rounded px-2.5 py-1 text-xs font-black transition ${
                                previewChannel === "rubika"
                                  ? "bg-app-primary text-white"
                                  : "bg-app-surfaceMuted text-app-muted hover:bg-slate-200"
                              }`}
                            >
                              روبیکا
                            </button>
                        ...
                        {previewChannel === "instagram" ? (
                          <div className="flex items-center justify-between border-t border-app-border pt-2">
                            <p className="text-xs font-black text-app-muted">
                              قالب انتشار
                            </p>
                            <div className="flex gap-1">
                              {(["feed", "story", "reel"] as const).map(
                                (mode) => (
                                  <button
                                    key={mode}
                                    type="button"
                                    onClick={() =>
                                      setInstagramPreviewMode(mode)
                                    }
                                    className={`rounded px-2 py-0.5 text-[10px] font-bold capitalize transition ${
                                      instagramPreviewMode === mode
                  ...
                  {studioPanel === "review" ? (
                    <div>
                      <div className="mb-3 flex items-center justify-between gap-2">
                        ...
                        <StatusToken tone={canSchedule ? "success" : "warning"}>
                          {canSchedule ? "آماده صف" : "نیازمند تکمیل"}
                        </StatusToken>
                      </div>
                      {editingPost ? (
                        <div className="mb-3 rounded-md border border-app-border bg-app-surfaceMuted/70 p-3">
                          ...
                          <p className="mt-2 text-xs leading-5 text-app-muted">
                            {
                              approvalConfig(editingPost.approval_status)
                                .description
                            }
                          </p>
                          {editingPost.approval_note ? (
                            <p className="mt-2 rounded bg-white px-3 py-2 text-xs leading-5 text-app-muted shadow-hairline">
                              {editingPost.approval_note}
                            </p>
                          ) : null}
                        </div>
