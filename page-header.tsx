...
export function ComposerSchedulePanel({
  ...
  const [draft, setDraft] = useState<JalaliPickerParts>(() =>
    getJalaliPickerParts(scheduledAt, scheduleTimezone),
  );
  const [open, setOpen] = useState(false);
  const hasSchedule = Boolean(scheduledAt);
  const selectedParts = hasSchedule
    ? getJalaliPickerParts(scheduledAt, scheduleTimezone)
    : null;
    ...
    <div className="space-y-4" dir="rtl">
      <div className="relative">
        ...
        >
          <span className="min-w-0">
            <span className="block text-xs font-black text-app-muted">
              زمان انتشار
            </span>
            <span className="mt-1 block truncate text-sm font-black text-app-text">
              {formatSchedule(selectedParts)}
            </span>
          </span>
          <span
            className={`rounded-md px-3 py-2 text-xs font-black ${hasSchedule ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-app-primary"}`}
          >
              ...
              {dayCells.map((day, index) => {
                ...
                const classes = selected
                  ...
                  : today
                    ...
                    : "bg-slate-50 text-slate-700 hover:bg-blue-50 hover:text-app-primary";
                return (
                  <button
                    key={day}
                    type="button"
                    onClick={() => emit({ ...draft, day })}
                    className={`h-7 rounded text-[11px] font-black transition ${classes}`}
                  >
                    {day}
                  </button>
                );
            ...
            <div className="mt-2 grid grid-cols-2 gap-1.5">
              <label className="text-[10px] font-black text-app-muted">
                ...
                <select
                  ...
                  className="mt-1 h-8 w-full rounded-md border border-app-border bg-white px-2 text-xs font-bold text-app-text outline-none ring-app-primary focus:ring-2"
                >
      ...
      <div className="rounded-md bg-app-surfaceMuted p-3 text-sm leading-7 text-app-muted shadow-hairline">
        <p>
          ...
          {hasSchedule
            ? "با ذخیره فرم، پست زمان‌بندی می‌شود."
            : "فعلاً به عنوان پیش‌نویس ذخیره می‌شود."}
        </p>
        <p>
          <span className="font-semibold text-app-text">زمان انتخاب‌شده:</span>{" "}
          {formatSchedule(selectedParts)}
        </p>
      </div>
    </div>
