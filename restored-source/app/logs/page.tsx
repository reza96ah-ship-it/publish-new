...
function attemptMode(requestPayload: ParsedPayload): Exclude<LogMode, "all"> {
  ...
  if (requestPayload?.mode === "media" || requestPayload?.media_asset_id)
    return "media";
  return "text";
}
...
function buildTimeline(
  ...
  if (mode === "media") {
    const filename =
      payloadText(requestPayload, "filename") || "رسانه پیوست‌شده";
    const contentType =
      payloadText(requestPayload, "content_type") || "نوع نامشخص";
    ...
    stages.push(
      ...
      },
      {
        ...
        detail: responseHas("upload_request")
          ? "Rubika آدرس آپلود را برگرداند."
          : "در انتظار پاسخ requestSendFile یا ثبت خطا.",
        state: responseHas("upload_request")
          ? "done"
      ...
      {
        label: "آپلود فایل",
        detail: responseHas("upload")
          ? `فایل آپلود شد${payloadText(responsePayload, "file_id") ? ` · file_id: ${payloadText(responsePayload, "file_id")}` : ""}`
          : "آپلود هنوز در payload موفق ثبت نشده است.",
        state: responseHas("upload")
          ? "done"
          : attempt.status === "failed"
            ? "pending"
            : "active",
      ...
      {
        label: "ارسال فایل",
        detail: responseHas("send")
          ? "sendFile با پاسخ موفق ثبت شد."
          : "ارسال نهایی فایل هنوز کامل نشده است.",
        state: responseHas("send")
          ? "done"
          : attempt.status === "failed"
            ? "pending"
            : "active",
    ...
    stages.push({
      ...
      detail: textLength
        ? `${textLength} کاراکتر برای Rubika آماده شد.`
        : "متن از کپشن، هشتگ یا عنوان پست ساخته شد.",
      state:
...
function timelineTone(
  state: TimelineStageState,
): "neutral" | "primary" | "success" | "warning" | "alert" {
...
export default function LogsPage() {
  ...
  const loadAttempts = useCallback(async (quiet = false) => {
    ...
    setError("");
    try {
      const response = await fetch(`${apiUrl}/publish-attempts`, {
        headers: authHeaders(),
      });
  ...
  async function retryPost(postId: number, title: string) {
    ...
    setMessage(
      "پست دوباره وارد صف انتشار شد. تلاش جدید پس از اجرای worker در این فهرست نمایش داده می‌شود.",
    );
    setRetryingPostId(null);
    showToast({
  ...
  const summary = useMemo(() => {
    return {
      ...
      started: preparedAttempts.filter(
        (item) => item.attempt.status === "started",
      ).length,
      success: preparedAttempts.filter(
        (item) => item.attempt.status === "success",
      ).length,
      failed: preparedAttempts.filter(
        (item) => item.attempt.status === "failed",
      ).length,
  ...
  const healthSummary = [
    ...
    },
    {
      ...
      icon: AlertTriangle,
      tone: summary.failed ? "text-rose-700" : "text-slate-500",
    },
  ];
            ...
            <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
              <div>
                ...
                <h1 className="mt-1 text-xl font-black text-app-text">
                  پایش تلاش‌های انتشار
                </h1>
                <p className="mt-1 text-xs leading-5 text-app-muted">
                  خطاها، مراحل ارسال و payloadهای فنی را در یک مسیر متمرکز بررسی
                  کنید.
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <StatusToken tone={summary.failed ? "alert" : "success"}>
                  {summary.failed
                    ? `${summary.failed} خطای فعال`
                    : "انتشار پایدار"}
                </StatusToken>
                <StatusToken tone="success">{successRate}% موفقیت</StatusToken>
                <StatusToken tone="neutral">
                  {summary.media} رسانه‌ای
                </StatusToken>
                {lastUpdatedAt ? (
                  <StatusToken tone="neutral">
                    به‌روزرسانی{" "}
          ...
          <section className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_340px]">
            <div className="min-w-0">
              <WorkspacePanel
                ...
                description="آخرین تلاش‌ها را اسکن کنید و جزئیات فنی را در بازرس کناری ببینید."
              ...
              >
                <DataToolbar
                  meta={
                    <>
                      <StatusToken tone="neutral">
                        {visibleAttempts.length} نتیجه
                      </StatusToken>
                      <StatusToken tone="neutral">
                        {summary.total} کل تلاش
                      </StatusToken>
                    </>
                  }
                >
                        ...
                        <FilterChip
                          ...
                          count={
                            filter.value === "all"
                              ...
                              : preparedAttempts.filter(
                                  (item) => item.mode === filter.value,
                                ).length
                          }
                ...
                >
                  {visibleAttempts.map(({ attempt, mode }) => {
                    ...
                    return (
                      <DataRow
                        key={attempt.id}
                        gridClassName={logsRowGrid}
                        selected={selected}
                      >
                        <div className="min-w-0">
                          <h2 className="truncate text-sm font-black text-app-text sm:text-base">
                            {attempt.post_title}
                          </h2>
                          <p className="mt-1 text-xs text-app-muted">
                            Post #{attempt.post_id} · Attempt #{attempt.id}
                          </p>
                          {attempt.error ? (
                            <p className="mt-2 line-clamp-2 rounded bg-rose-50 p-2 text-xs leading-5 text-rose-700">
                              {attempt.error}
                            </p>
                          ) : null}
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 lg:block lg:space-y-2">
                          ...
                          <StatusToken
                            tone={mode === "media" ? "primary" : "neutral"}
                            className="gap-1"
                          >
                            {mode === "media" ? (
                              <FileUp
            ...
            <aside className="space-y-3 lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] lg:self-start lg:overflow-y-auto">
              <WorkspacePanel
                ...
                action={
                  selectedAttempt ? (
                    <StatusBadge
                      status={attemptTone(selectedAttempt.attempt.status)}
                    />
                  ) : null
                }
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        />
                        <ChannelBadges
                          platform={selectedAttempt.attempt.channel}
                          compact
                        />
                        <StatusToken
                          tone={
                        ...
                        <StatusToken tone="neutral">
                          {actionLabel(selectedAttempt.attempt.action)}
                        </StatusToken>
                      </div>
                      <h3 className="mt-3 text-lg font-black text-app-text">
                        {selectedAttempt.attempt.post_title}
                      </h3>
                      <p className="mt-2 text-xs leading-6 text-app-muted">
                        Post #{selectedAttempt.attempt.post_id} · Attempt #
                      ...
                      items={[
                        {
                          ...
                          value: formatDateTime(
                            selectedAttempt.attempt.started_at ||
                              selectedAttempt.attempt.created_at,
                          ),
                          hint: "زمان ثبت تلاش",
                        },
                        {
                          label: "پایان",
                          value: formatDateTime(
                            selectedAttempt.attempt.finished_at,
                          ),
                          hint: "زمان پایان تلاش",
                        },
                        {
                          label: "نوع",
                          value: modeLabel(selectedAttempt.mode),
                          hint: "متنی یا رسانه‌ای",
                        },
                        {
                          label: "عملیات",
                          value: actionLabel(selectedAttempt.attempt.action),
                          hint: "منبع اجرای تلاش",
                        },
                      ]}
                    />
                    ...
                    <div>
                      ...
                      <Timeline
                        items={selectedAttempt.timeline.map((stage) => ({
                          title: stage.label,
                        ...
                        items={[
                          {
                            ...
                            value:
                              payloadText(
                                selectedAttempt.requestPayload,
                                "filename",
                              ) || "—",
                            hint: "نام فایل",
                          },
                          ...
                          },
                          {
                            ...
                            value:
                              payloadText(
                                selectedAttempt.responsePayload,
                                "file_id",
                              ) || "—",
                            hint: "شناسه کانال",
                          },
                    ...
                    <details className="rounded-md border border-app-border bg-slate-50 p-3 text-xs text-app-muted">
                      ...
                      <pre className="mt-3 max-h-80 overflow-auto whitespace-pre-wrap break-words leading-6">
                        Request:{" "}
                        {prettyPayload(selectedAttempt.attempt.request_payload)}
                        {"\n\n"}Response:{" "}
                        {prettyPayload(
                          selectedAttempt.attempt.response_payload,
                        )}
                      </pre>
                    </details>
                    ...
                    <div className="grid gap-2">
                      {selectedAttempt.attempt.status === "failed" ? (
                        <Button
                          type="button"
                          disabled={
                            retryingPostId === selectedAttempt.attempt.post_id
                          }
                          onClick={() =>
                            retryPost(
                              selectedAttempt.attempt.post_id,
                              selectedAttempt.attempt.post_title,
                            )
                          }
                        >
                          <RotateCcw
                            className={`ml-2 h-4 w-4 ${retryingPostId === selectedAttempt.attempt.post_id ? "animate-spin" : ""}`}
                            aria-hidden="true"
                          />
                          {retryingPostId === selectedAttempt.attempt.post_id
                            ? "در حال ورود به صف"
                      ...
                      <Button
                        href={`/compose?postId=${selectedAttempt.attempt.post_id}`}
                        variant="secondary"
                      >
                        باز کردن پست
                      </Button>
