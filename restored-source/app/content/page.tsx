...
const searchableFields: Array<
  keyof Pick<
    Post,
    "title" | "caption" | "hashtags" | "campaign" | "internal_note"
  >
> = ["title", "caption", "hashtags", "campaign", "internal_note"];
...
export default function ContentWorkspacePage() {
  ...
  const [reviewingAction, setReviewingAction] = useState<ReviewAction | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (
      requestedStatus &&
      workflowTabs.some((tab) => tab.value === requestedStatus)
    ) {
  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts
      ...
      .filter(
        (post) =>
          approvalFilter === "all" ||
          (post.approval_status || "not_required") === approvalFilter,
      )
  ...
  const savedViews = useMemo(
    () => [
      { label: "همه", value: "all", count: posts.length },
      {
        ...
        value: "attention",
        count: posts.filter((post) => postMatchesSavedView(post, "attention"))
          .length,
      },
      {
        label: "آماده انتشار",
        value: "ready",
        count: posts.filter((post) => postMatchesSavedView(post, "ready"))
          .length,
      },
      {
        label: "پیش‌نویس",
        value: "draft",
        count: posts.filter((post) => postMatchesSavedView(post, "draft"))
          .length,
      },
      {
        label: "منتشرشده",
        value: "published",
        count: posts.filter((post) => postMatchesSavedView(post, "published"))
          .length,
      },
    ],
    [posts],
  ...
  const metrics = useMemo<Metric[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: failed,
        hint: failed
          ? "پست ناموفق یا خطادار را بازبینی کنید"
          : "خطای فعال ندارید",
        icon: AlertCircle,
      ...
      },
      {
        ...
        value: pendingReview,
        hint: pendingReview
          ? "تایید یا درخواست اصلاح را از پنل بازبینی انجام دهید"
          : "هیچ پست منتظر بازبینی نیست",
        icon: ShieldCheck,
      ...
      },
      {
        ...
        hint: nextScheduled
          ? `نزدیک‌ترین انتشار: ${formatDateTime(nextScheduled.scheduled_at)}`
          : "هنوز انتشار آینده ثبت نشده",
        icon: CalendarClock,
        tone: "warning",
      },
    ];
  ...
  const allVisibleSelected =
    ...
    selectedVisibleIds.every((id) => selectedIds.has(id));
  ...
  async function bulkChangeStatus(status: "ready" | "cancelled") {
    ...
    if (!response.ok) {
      const detail = await readApiError(
        response,
        "به‌روزرسانی گروهی محتوا ناموفق بود",
      );
      setError(detail);
    ...
    const skippedText = result.skipped_post_ids.length
      ? `، ${result.skipped_post_ids.length} مورد بدون تغییر باقی ماند`
      : "";
    setMessage(`${result.updated_count} پست به‌روزرسانی شد${skippedText}`);
    showToast({
      title: "عملیات گروهی انجام شد",
      description: `${result.updated_count} پست به‌روزرسانی شد`,
      tone: "success",
    });
  ...
  async function changeStatus(post: Post, status: string) {
      ...
      current.map((item) =>
        item.id === post.id
          ? {
              ...item,
              status,
              scheduled_at: status === "cancelled" ? null : item.scheduled_at,
            }
          : item,
      ),
    );
    ...
    showToast({
      title: "وضعیت پست به‌روزرسانی شد",
  ...
  function canSubmitForReview(post: Post) {
    return (
      ["draft", "ready", "failed", "cancelled"].includes(post.status) &&
  ...
  async function reviewPost(post: Post, action: ReviewAction) {
    ...
    if (!response.ok) {
      ...
      showToast({
        title: "عملیات بازبینی ناموفق بود",
        description: detail,
        tone: "alert",
      });
      return;
    ...
    <AuthGate>
      <AppShell>
        <NPage className="content-ops-page pb-5">
          <NPageHeader
            ...
            meta={
              <>
                ...
                <NStatusPill tone="success">
                  {publishedCount} منتشرشده
                </NStatusPill>
                <NStatusPill tone={failedCount ? "alert" : "success"}>
                  {failedCount
                    ? `${failedCount} نیازمند رسیدگی`
                    : "بدون خطای فعال"}
                </NStatusPill>
                <NStatusPill tone="warning">
                  {scheduledCount} زمان‌بندی‌شده
                </NStatusPill>
                {lastUpdatedAt ? (
                  <NStatusPill tone="neutral">
                    به‌روزرسانی{" "}
          ...
          {message ? (
            <NNotice tone="success" title="انجام شد">
              {message}
            </NNotice>
          ) : null}
          {error ? (
                    ...
                    >
                      {workflowTabs.map((tab) => (
                        <option key={tab.value} value={tab.value}>
                          {tab.label} · {statusCount(posts, tab.value)}
                        </option>
                      ))}
                    </select>
                  </label>
              ...
              <div className="mt-4 flex flex-col justify-between gap-3 rounded-md border border-blue-200 bg-blue-50 px-3 py-3 sm:flex-row sm:items-center">
                <div>
                  <p className="text-sm font-black text-app-text">
                    {selectedIds.size} پست انتخاب شده است
                  </p>
                  <p className="mt-1 text-xs leading-5 text-app-muted">
                    عملیات گروهی فقط روی وضعیت‌های مجاز اجرا می‌شود و موارد
                    ناسازگار بدون تغییر باقی می‌مانند.
                  </p>
                </div>
                <div className="flex shrink-0 flex-wrap gap-2">
                  <Button
                    ...
                    onClick={() => bulkChangeStatus("ready")}
                  >
                      ...
                      <ContentOperationCard
                        ...
                        meta={
                          <>
                            ...
                            <p>
                              به‌روزرسانی: {formatDateTime(post.updated_at)}
                            </p>
                          </>
                        }
          ...
          <NInspectorDrawer
            ...
            footer={
              selectedPost ? (
                <ApprovalBadge status={selectedPost.approval_status} compact />
              ) : null
            }
          >
            {selectedPost ? (
              <div className="space-y-4">
                <div className="overflow-hidden rounded-md bg-slate-50 ring-1 ring-app-border">
                  {previewUrlForPost(selectedPost) ? (
                    <img
                      ...
                      alt={
                        primaryMediaForPost(selectedPost)?.original_filename ??
                        selectedPost.title
                      }
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {primaryMediaForPost(selectedPost) ? (
                      ...
                      <StatusToken tone="warning">نیازمند رسانه</StatusToken>
                    )}
                  </div>
                ...
                <div className="max-h-72 overflow-auto whitespace-pre-wrap rounded-md border border-app-border bg-slate-50 p-4 text-sm leading-7 text-slate-700">
                  {postFinalText(selectedPost) ||
                    "متن نهایی برای این پست هنوز کامل نشده است."}
                </div>
                  ...
                  items={[
                    {
                      ...
                      hint: "زمان برنامه‌ریزی انتشار",
                    },
                    {
                      label: "کمپین",
                      value: campaignLabelForPost(selectedPost, campaigns),
                      hint: "برچسب عملیاتی محتوا",
                    },
                    {
                      label: "بازبینی",
                      value: approvalConfig(selectedPost.approval_status).label,
                      hint: approvalConfig(selectedPost.approval_status)
                        .description,
                    },
                    {
                      label: "بازبین",
                      value: selectedPost.reviewed_by || "ثبت نشده",
                      hint: selectedPost.reviewed_at
                        ? formatDateTime(selectedPost.reviewed_at)
                        : "هنوز تصمیم نهایی ثبت نشده",
                    },
                    {
                      label: "تلاش انتشار",
                ...
                <section className="rounded-md border border-app-border bg-app-surfaceMuted/70 p-3">
                  ...
                  {selectedPost.submitted_at ? (
                    <p className="mt-3 text-xs text-app-muted">
                      ارسال برای بازبینی:{" "}
                      {formatDateTime(selectedPost.submitted_at)}
                    </p>
                  ) : null}
                  <textarea
                    value={reviewNote}
                  ...
                  <div className="mt-3 grid gap-2">
                    {canSubmitForReview(selectedPost) ? (
                      <Button
                        ...
                        onClick={() =>
                          reviewPost(selectedPost, "submit-review")
                        }
                      >
                        <ShieldCheck
                    ...
                    {canReviewDecision(selectedPost) ? (
                      <div className="grid gap-2 sm:grid-cols-3">
                        <Button
                          ...
                          size="sm"
                          disabled={Boolean(reviewingAction)}
                          onClick={() => reviewPost(selectedPost, "approve")}
                        >
                          <ThumbsUp
                        ...
                        <Button
                          ...
                          disabled={Boolean(reviewingAction)}
                          onClick={() =>
                            reviewPost(selectedPost, "request-changes")
                          }
                        >
                          <MessageSquareText
                            className="ml-1.5 h-3.5 w-3.5"
                        ...
                        <Button
                          ...
                          size="sm"
                          disabled={Boolean(reviewingAction)}
                          onClick={() => reviewPost(selectedPost, "reject")}
                        >
                          <ThumbsDown
                            className="ml-1.5 h-3.5 w-3.5"
                            aria-hidden="true"
                ...
                <div className="grid gap-2">
                  ...
                  selectedPost.status === "cancelled" ? (
                    <Button
                      type="button"
                      variant="secondary"
                      onClick={() => changeStatus(selectedPost, "ready")}
                    >
                      <CheckCircle2
                        className="ml-2 h-4 w-4"
                        aria-hidden="true"
                  ...
                  selectedPost.status !== "published" ? (
                    <Button
                      type="button"
                      variant="ghost"
                      onClick={() => changeStatus(selectedPost, "cancelled")}
                    >
                      <XCircle className="ml-2 h-4 w-4" aria-hidden="true" />
                      لغو پست
                    </Button>
