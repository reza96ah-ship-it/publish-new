...
const campaignWorkbenchTabs: Array<{
  id: CampaignWorkbenchTab;
  label: string;
  helper: string;
}> = [
...
function campaignToForm(campaign: Campaign): CampaignForm {
  return {
    name: campaign.name,
    goal: campaign.goal,
    status: campaignStatusOptions.some(
...
function sameJalaliDay(
  parts: JalaliPickerParts | null,
  year: number,
  month: number,
  day: number,
) {
...
function CampaignJalaliDateField({
  ...
  const [draft, setDraft] = useState<JalaliPickerParts>(() =>
    getJalaliPickerParts(value, timezone),
  );
  ...
  function moveMonth(delta: number) {
    ...
    setDraft({
      ...
      year: nextYear,
      month: nextMonth,
      day: Math.min(draft.day, nextLength),
    });
  }
    ...
    <div ref={rootRef} className="relative" dir="rtl">
      ...
      >
        <span className="min-w-0">
          ...
          <span className="mt-1 block truncate text-[11px] font-bold text-app-muted">
            {formatJalaliSelection(value)}
          </span>
        </span>
        <span
          className={`nashrino-control-radius inline-flex min-h-8 items-center px-3 text-[11px] font-black ${value ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-app-primary"}`}
        >
          {value ? "تغییر" : "انتخاب"}
        </span>
      </button>
      ...
      {open ? (
        <div className="app-popover absolute bottom-full right-0 z-[70] mb-2 w-[244px] rounded-lg border border-app-border bg-white p-2.5 shadow-lift">
          <div className="flex items-center justify-between gap-1.5">
            ...
            <p className="min-w-20 text-center text-xs font-black text-app-primary">
          ...
          <div className="mt-1 grid grid-cols-7 gap-0.5">
            {dayCells.map((day, index) => {
              ...
              const selected = sameJalaliDay(
                selectedParts,
                draft.year,
                draft.month,
                day,
              );
              const today = sameJalaliDay(
                todayParts,
                draft.year,
                draft.month,
                day,
          ...
          <div className="mt-2 grid grid-cols-2 gap-1.5">
            <label className="text-[10px] font-black text-app-muted">
              ...
              <select
                ...
                className="mt-1 h-8 w-full rounded-md border border-app-border bg-white px-2 text-xs font-bold text-app-text outline-none focus:ring-2 focus:ring-blue-100"
              >
...
function riskScoreForPost(post: Post, hasMedia: boolean) {
  ...
  if (post.attempt_count > 1)
    score += Math.min(24, (post.attempt_count - 1) * 8);
  if (!hasMedia) score += 8;
...
function buildCampaignStats(
  ...
  const media = posts.filter(
    (post) => (mediaByPostId.get(post.id) ?? []).length > 0,
  ).length;
...
export default function CampaignsPage() {
  ...
  const [activeCampaignTab, setActiveCampaignTab] =
    useState<CampaignWorkbenchTab>("overview");
  const [assignmentOpen, setAssignmentOpen] = useState(false);
  const [campaignForm, setCampaignForm] =
    useState<CampaignForm>(emptyCampaignForm);
  const [campaignColorDraft, setCampaignColorDraft] = useState(
    emptyCampaignForm.color,
  );
  const [openCampaignCalendar, setOpenCampaignCalendar] =
    useState<CampaignDateField | null>(null);
  ...
  const loadWorkspace = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      const [campaignResponse, postsResponse, mediaResponse, attemptsResponse] =
        await Promise.all([
          loadCampaigns(),
      ...
      if (!postsResponse.ok)
        throw new Error("دریافت پست‌ها برای کمپین ناموفق بود");
      const nextPosts = (await postsResponse.json()) as Post[];
      const nextMedia = mediaResponse.ok
        ? ((await mediaResponse.json()) as MediaAsset[])
        : [];
      const nextAttempts = attemptsResponse.ok
        ? ((await attemptsResponse.json()) as PublishAttempt[])
        : [];
      ...
      setSelectedCampaignId(
        (current) => current ?? campaignResponse[0]?.id ?? null,
      );
  ...
  useEffect(() => {
    ...
    const routedCampaign = campaigns.find(
      (campaign) => String(campaign.id) === routeCampaignId,
    );
    ...
    return campaigns
      .map((campaign) => {
        ...
        return {
          campaign,
          posts: linkedPosts,
          stats: buildCampaignStats(linkedPosts, mediaByPostId),
        };
      })
      .filter((row) => {
        if (!query) return true;
        return `${row.campaign.name} ${row.campaign.goal} ${row.campaign.owner} ${row.campaign.notes}`
          .toLowerCase()
          .includes(query);
      })
      .sort((first, second) => {
        if (
          first.campaign.status === "active" &&
          second.campaign.status !== "active"
        )
          return -1;
        if (
          first.campaign.status !== "active" &&
          ...
          return 1;
        return (
          second.stats.health - first.stats.health ||
          second.stats.total - first.stats.total
        );
      });
  ...
  const selectedRow = useMemo(() => {
    ...
    return (
      campaignRows.find((row) => row.campaign.id === selectedCampaignId) ??
      campaignRows[0] ??
      null
    );
  ...
  useEffect(() => {
    setCampaignColorDraft(
      /^#[0-9A-Fa-f]{6}$/.test(campaignForm.color)
        ? campaignForm.color
        : emptyCampaignForm.color,
    );
  }, [campaignForm.color]);
  ...
  const selectedPosts = useMemo(() => {
    return [...(selectedRow?.posts ?? [])].sort(
      (first, second) => postActivityTime(second) - postActivityTime(first),
    );
  }, [selectedRow]);
  ...
  const selectedAssets = useMemo(() => {
    ...
    return mediaAssets.filter(
      (asset) => asset.post_id && ids.has(asset.post_id),
    );
  ...
  const campaignInsights = useMemo(() => {
    const published = selectedPosts.filter(
      (post) => post.status === "published",
    ).length;
    const failed = selectedPosts.filter(
      (post) => post.status === "failed" || post.last_error,
    ).length;
    const queued = selectedPosts.filter(isQueued).length;
    const draft = selectedPosts.filter(
      (post) => post.status === "draft",
    ).length;
    const withMedia = selectedPosts.filter(
      (post) => (mediaByPostId.get(post.id) ?? []).length > 0,
    ).length;
    const successfulAttempts = selectedAttempts.filter(
      (attempt) => attempt.status === "success",
    ).length;
    const failedAttempts = selectedAttempts.filter(
      (attempt) => attempt.status === "failed",
    ).length;
    const completedAttempts = successfulAttempts + failedAttempts;
    const recentPublished = selectedPosts.filter(
      (post) =>
        post.status === "published" && isWithinDays(post.published_at, 7),
    ).length;
    const recentFailed = selectedPosts.filter(
      (post) =>
        (post.status === "failed" || post.last_error) &&
        isWithinDays(post.failed_at || post.updated_at, 7),
    ).length;
    ...
    // This value is purely used to initialize state so hydration matches
    // But since it's used inside useMemo, let's just make it ignore hydration mismatches on the render container instead.
    const trend = trendKeys.map((key) => {
      const dayPosts = selectedPosts.filter(
        (post) =>
          dayKey(
            post.published_at ||
              post.failed_at ||
      ...
      return {
        ...
        published: dayPosts.filter((post) => post.status === "published")
          .length,
        failed: dayPosts.filter(
          (post) => post.status === "failed" || post.last_error,
        ).length,
      };
    });
    ...
    const riskPosts = selectedPosts
      .map((post) => ({
        post,
        score: riskScoreForPost(
          post,
          (mediaByPostId.get(post.id) ?? []).length > 0,
        ),
      }))
      ...
      .sort(
        (first, second) =>
          ...
          postActivityTime(second.post) - postActivityTime(first.post),
      )
      .slice(0, 5);
    const statusMix = [
  ...
  const assignablePosts = useMemo(() => {
    ...
    return posts
      ...
      .filter((post) => {
        if (!query) return true;
        return `${post.title} ${post.caption} ${post.hashtags} ${post.campaign} ${post.status}`
          .toLowerCase()
          .includes(query);
      })
      .sort(
  ...
  const queuedCount = selectedRow
    ? selectedRow.stats.ready +
      selectedRow.stats.scheduled +
      selectedRow.stats.publishing
    : 0;
  const healthTone: "success" | "warning" | "alert" =
    (selectedRow?.stats.health ?? 0) >= 76
      ? "success"
      : (selectedRow?.stats.health ?? 0) >= 50
        ? "warning"
        : "alert";
  ...
  function toggleAllAssignable() {
    ...
    const allSelected =
      visibleIds.length > 0 &&
      visibleIds.every((id) => selectedAssignIds.has(id));
    setSelectedAssignIds((current) => {
      const next = new Set(current);
  ...
  function updateCampaignField<K extends keyof CampaignForm>(
    field: K,
  ...
  function focusCampaignEditor() {
    window.setTimeout(() => {
      document
        .getElementById("campaign-editor")
  ...
  async function saveCampaign(event: FormEvent<HTMLFormElement>) {
    ...
    if (!name) {
      ...
      showToast({
        title: "نام کمپین لازم است",
        description: "برای ذخیره کمپین، یک نام مشخص وارد کنید.",
        tone: "warning",
      });
      return;
    }
    ...
    try {
      ...
      setMessage(
        editorMode === "create"
          ? "کمپین جدید ساخته شد."
          : "کمپین به‌روزرسانی شد.",
      );
      showToast({
        title: editorMode === "create" ? "کمپین ساخته شد" : "کمپین ذخیره شد",
        description: savedCampaign.name,
        tone: "success",
      });
    } catch (err) {
      const nextError =
        err instanceof Error ? err.message : "ذخیره کمپین ناموفق بود";
      setError(nextError);
      showToast({
        title: "ذخیره کمپین ناموفق بود",
  ...
  async function assignSelectedPosts() {
    ...
    setMessage("");
    try {
      const result = await assignPostsToCampaign(
        [...selectedAssignIds],
        selectedRow.campaign.id,
      );
      setPosts((current) =>
        current.map((post) =>
          result.post_ids.includes(post.id)
            ? {
                ...post,
                campaign_id: selectedRow.campaign.id,
                campaign: selectedRow.campaign.name,
              }
            : post,
        ),
      );
      setSelectedAssignIds(new Set());
      setAssignmentOpen(false);
      const skippedText = result.skipped_post_ids.length
        ? `، ${result.skipped_post_ids.length} مورد رد شد`
        : "";
      setMessage(`${result.updated_count} پست به کمپین وصل شد${skippedText}.`);
      showToast({
        title: "پست‌ها به کمپین وصل شدند",
        description: `${result.updated_count} پست به ${selectedRow.campaign.name} اضافه شد`,
        tone: "success",
      });
    } catch (err) {
      const nextError =
        err instanceof Error ? err.message : "اتصال پست‌ها به کمپین ناموفق بود";
      setError(nextError);
      showToast({
        title: "اتصال گروهی ناموفق بود",
        description: nextError,
        tone: "alert",
      });
    } finally {
  ...
  async function removePostFromCampaign(post: Post) {
    ...
    try {
      ...
      setPosts((current) =>
        current.map((item) =>
          result.post_ids.includes(item.id)
            ? { ...item, campaign_id: null, campaign: "" }
            : item,
        ),
      );
      setMessage("پست از کمپین جدا شد.");
      showToast({
        title: "پست از کمپین جدا شد",
        description: post.title,
        tone: "success",
      });
    } catch (err) {
      const nextError =
        err instanceof Error ? err.message : "جدا کردن پست از کمپین ناموفق بود";
      setError(nextError);
      showToast({
        title: "جدا کردن پست ناموفق بود",
        description: nextError,
        tone: "alert",
      });
    } finally {
  ...
  function buildCampaignReportRows() {
    ...
    return [
      ...
      [
        "بازه",
        `${formatDateTime(campaign.starts_at)} تا ${formatDateTime(campaign.ends_at)}`,
      ],
      ...
      ...(campaignInsights.riskPosts.length
        ? campaignInsights.riskPosts.map(({ post, score }) => [
            ...
            post.last_error || `${post.attempt_count} تلاش ثبت‌شده`,
          ])
  ...
  function exportCampaignCsv() {
    ...
    downloadTextFile(
      filename,
      `\ufeff${toCsv(buildCampaignReportRows())}`,
      "text/csv;charset=utf-8",
    );
    showToast({
      title: "گزارش CSV آماده شد",
      description: selectedRow.campaign.name,
      tone: "success",
    });
  }
  ...
  function exportCampaignHtml() {
    ...
    const statusRows = campaignInsights.statusMix
      .map(
        (item) => `
    ...
    `,
      ...
      .join("");
    const trendRows = campaignInsights.trend
      .map(
        (point) => `
    ...
    const priorityRows = (
      campaignInsights.riskPosts.length
        ? campaignInsights.riskPosts.map(
            ({ post, score }) => `
      ...
      `,
        ...
        : [
            `<tr><td colspan="4">ریسک فعالی برای پست‌های این کمپین دیده نمی‌شود.</td></tr>`,
          ]
    ).join("");
    const postRows = selectedPosts
      .map(
        (post) => `
      <tr>
...
<body>
  <main>
    ...
    showToast({
      title: "گزارش HTML آماده شد",
      description: "فایل را می‌توانید چاپ یا ارسال کنید.",
      tone: "success",
    });
  }
        ...
        <WorkspacePage className="campaigns-pro-page">
          <PublishingWorkspaceHeader
            ...
            counts={{
              ...
              queue: posts.filter(isQueued).length,
            }}
            meta={
              <>
                <StatusToken tone="primary">
                  {activeCount} کمپین فعال
                </StatusToken>
                <StatusToken tone="neutral">
                  {campaigns.length} کل کمپین
                </StatusToken>
                <StatusToken
                  tone={
                    posts.some((post) => post.status === "failed")
                      ? "alert"
                      : "success"
                  }
                >
          ...
          {message ? (
            <NoticeBanner tone="success" title="انجام شد">
              {message}
            </NoticeBanner>
          ) : null}
          ...
          {selectedRow ? (
            <>
              <section
                className="campaign-command-strip"
                style={
                  {
                    "--campaign-accent": selectedRow.campaign.color,
                  } as CSSProperties
                }
              >
                <div className="campaign-command-copy">
                  ...
                  <p>
                    {selectedRow.campaign.goal ||
                      selectedRow.campaign.notes ||
                      "هدف کمپین را کامل کنید تا برنامه انتشار دقیق‌تر شود."}
                  </p>
                </div>
                <div className="campaign-command-actions">
                  <Button
                    href={`/calendar?campaignId=${selectedRow.campaign.id}`}
                    size="sm"
                  >
                    <CalendarDays
                  ...
                  <Button
                    href={`/compose?campaignId=${selectedRow.campaign.id}`}
                    variant="secondary"
                    size="sm"
                  >
                  ...
                  <Button
                    href={`/media?campaignId=${selectedRow.campaign.id}`}
                    variant="ghost"
                    size="sm"
                  >
                    رسانه‌ها
              ...
              <section
                className="dashboard-kpi-strip campaign-command-metrics grid grid-cols-2 gap-3 lg:grid-cols-4"
                aria-label="خلاصه کمپین انتخاب‌شده"
              >
                <MetricTile
                  label="پست‌ها"
                  value={selectedRow.stats.total}
                  detail="محتوای متصل به کمپین"
                  icon={Target}
                  tone="primary"
                />
                <MetricTile
                  label="سلامت"
                  value={`${selectedRow.stats.health}%`}
                  detail="کیفیت عملیاتی کمپین"
                  icon={CheckCircle2}
                  tone={healthTone}
                />
                <MetricTile
                  label="در جریان"
                  value={queuedCount}
                  detail="آماده، زمان‌بندی یا انتشار"
                  icon={TimerReset}
                  tone="info"
                />
                <MetricTile
            ...
            >
              <DataToolbar
                meta={
                  <>
                    <StatusToken tone="neutral">
                      {campaigns.length} کمپین
                    </StatusToken>
                    <StatusToken tone="neutral">
                      {posts.filter((post) => post.campaign_id).length} پست متصل
                    </StatusToken>
                  </>
                }
              >
                <DataSearchField
                  ...
                  placeholder="جست‌وجوی نام، هدف، مالک یا یادداشت کمپین"
                />
              </DataToolbar>
                ...
                {campaignRows.map((row) => {
                  ...
                  return (
                    <button
                      ...
                      style={
                        {
                          "--campaign-accent": row.campaign.color,
                        } as CSSProperties
                      }
                    >
                      <span className="campaign-card-main">
                        ...
                        <span className="campaign-card-copy">
                          <span className="campaign-card-title-row">
                            ...
                            <StatusToken
                              tone={campaignStatusTone(row.campaign.status)}
                            >
                              {statusLabels[row.campaign.status] ??
                                row.campaign.status}
                            </StatusToken>
                          </span>
                          <span className="campaign-card-goal">
                            {row.campaign.goal ||
                              row.campaign.notes ||
                              "هدف کمپین هنوز تعریف نشده است."}
                          </span>
                        </span>
                      </span>
                      <span className="campaign-card-stats">
                        <span>
                          <strong>{row.stats.total}</strong> پست
                        </span>
                        <span>
                          <strong>{row.stats.published}</strong> منتشر
                        </span>
                        <span>
                          <strong
                            className={row.stats.failed ? "text-rose-700" : ""}
                          >
                            {row.stats.failed}
                          </strong>{" "}
                          خطا
            ...
            <aside className="campaign-management-stack">
                ...
                action={
                  selectedRow ? (
                    <Button
                      ...
                      variant="secondary"
                      size="sm"
                      onClick={() => openEditCampaign(selectedRow.campaign)}
                    >
                      ویرایش
                    </Button>
                  ) : null
              ...
              >
                {selectedRow ? (
                  <div className="campaign-selected-inspector">
                    <div
                      className="campaign-detail-hero-card"
                      style={
                        {
                          "--campaign-accent": selectedRow.campaign.color,
                        } as CSSProperties
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="text-[11px] font-black text-app-muted">
                            کمپین انتخاب‌شده
                          </p>
                          <h2 className="mt-1 truncate text-lg font-black text-app-text">
                            {selectedRow.campaign.name}
                          </h2>
                        </div>
                      ...
                      <p className="mt-3 text-sm leading-6 text-app-muted">
                        {selectedRow.campaign.goal ||
                          selectedRow.campaign.notes ||
                          "هدف و یادداشت کمپین هنوز تکمیل نشده است."}
                      </p>
                    </div>
                    ...
                    <div className="campaign-selected-facts">
                      <span>
                        <small>سلامت</small>
                        <strong>
                          <StatusToken tone={healthTone}>
                            {selectedRow.stats.health}%
                          </StatusToken>
                        </strong>
                      </span>
                      <span>
                        <small>وضعیت</small>
                        <strong>
                          {statusLabels[selectedRow.campaign.status] ??
                            selectedRow.campaign.status}
                        </strong>
                      </span>
                      <span>
                        <small>مالک</small>
                        <strong>
                          {selectedRow.campaign.owner || "تعریف نشده"}
                        </strong>
                      </span>
                      <span>
                        <small>بازه</small>
                        <strong>
                          {formatDateTime(selectedRow.campaign.starts_at)} تا{" "}
                          {formatDateTime(selectedRow.campaign.ends_at)}
                        </strong>
                      </span>
                    </div>
                    ...
                    <div className="campaign-progress-stack">
                      <div className="mb-2 flex items-center justify-between gap-3">
                        ...
                        <StatusToken tone="neutral">
                          {selectedRow.stats.total} پست
                        </StatusToken>
                      </div>
                      {[
                        {
                          label: "پوشش رسانه",
                          value: selectedRow.stats.media,
                          total: selectedRow.stats.total,
                          color: "rgb(var(--n-color-info))",
                        },
                        {
                          label: "در جریان انتشار",
                          value: queuedCount,
                          total: selectedRow.stats.total,
                          color: "rgb(var(--n-color-primary))",
                        },
                        {
                          label: "انتشار موفق",
                          value: selectedRow.stats.published,
                          total: selectedRow.stats.total,
                          color: "rgb(var(--n-color-success))",
                        },
                        {
                          label: "ریسک خطا",
                          value: failedCount,
                          total: selectedRow.stats.total,
                          color: "rgb(var(--n-color-alert))",
                        },
                      ].map((item) => {
                  ...
                  <EmptyState
                    title="کمپینی انتخاب نشده است"
                    description="از لیست کمپین‌ها یک مورد را انتخاب کنید."
                  />
                )}
              </WorkspacePanel>
              ...
              {editorOpen ? (
                <div
                  ...
                  aria-modal="true"
                  aria-label={
                    editorMode === "create" ? "ساخت کمپین" : "ویرایش کمپین"
                  }
                  onClick={closeCampaignEditor}
                >
                  <div
                    className="campaign-modal-panel campaign-editor-panel"
                  ...
                  >
                    <WorkspacePanel
                      ...
                      description={
                        editorMode === "create"
                          ? "کمپین جدید را با هدف، رنگ و مالک مشخص بسازید."
                          : "مشخصات عملیاتی کمپین انتخاب‌شده را به‌روزرسانی کنید."
                      }
                      bodyClassName="p-3 sm:p-4"
                      className="scroll-mt-24"
                      action={
                        editorMode === "create" ? (
                          <StatusToken tone="primary">جدید</StatusToken>
                        ) : selectedRow ? (
                          <StatusToken
                    ...
                    >
                      ...
                      >
                        <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_86px]">
                          <Field label="نام کمپین" required>
                            <Input
                              ...
                              onChange={(event) =>
                                updateCampaignField("name", event.target.value)
                              }
                              placeholder="مثلاً لانچ تابستان"
                              required
                            />
                          </Field>
                          <Field label="رنگ">
                            <Input
                              ...
                              onInput={(event) =>
                                setCampaignColorDraft(event.currentTarget.value)
                              }
                              onChange={(event) =>
                                setCampaignColorDraft(event.target.value)
                              }
                              onBlur={(event) =>
                                commitCampaignColor(event.currentTarget.value)
                              }
                              type="color"
                        ...
                        <Field
                          label="هدف کمپین"
                          hint="یک جمله واضح برای خروجی و سنجش کمپین."
                        >
                          <Textarea
                            value={campaignForm.goal}
                            onChange={(event) =>
                              updateCampaignField("goal", event.target.value)
                            }
                            className="min-h-20"
                            placeholder="افزایش فروش محصول، معرفی مجموعه جدید یا اطلاع‌رسانی رویداد..."
                          />
                        </Field>
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                          ...
                          <Field label="مالک">
                            <Input
                              ...
                              onChange={(event) =>
                                updateCampaignField("owner", event.target.value)
                              }
                              placeholder="نام مسئول کمپین"
                            />
                          </Field>
                        </div>
                        ...
                        <div className="grid gap-3">
                          <CampaignJalaliDateField
                            ...
                            onOpenChange={(nextOpen) =>
                              setOpenCampaignCalendar(
                                nextOpen ? "starts_at" : null,
                              )
                            }
                            onChange={(value) =>
                              updateCampaignField("starts_at", value)
                            }
                          />
                          <CampaignJalaliDateField
                            label="پایان کمپین"
                            ...
                            onOpenChange={(nextOpen) =>
                              setOpenCampaignCalendar(
                                nextOpen ? "ends_at" : null,
                              )
                            }
                            onChange={(value) =>
                              updateCampaignField("ends_at", value)
                            }
                          />
                        </div>
                        ...
                        <Field label="یادداشت">
                          <Textarea
                            ...
                            onChange={(event) =>
                              updateCampaignField("notes", event.target.value)
                            }
                            className="min-h-24"
                            placeholder="بودجه، فرضیه، نکته اجرایی یا تصمیم‌های تیم..."
                          />
                        </Field>
          ...
          {selectedRow ? (
            <section
              className="campaign-workbench-shell"
              aria-label="فضای کاری کمپین"
            >
              <div
                className="campaign-tabbar"
                role="tablist"
                ...
                <section className="campaign-insight-section" role="tabpanel">
                  <WorkspacePanel
                    ...
                    action={
                      <StatusToken
                        tone={
                          campaignInsights.recentFailed ? "alert" : "success"
                        }
                      >
                        {campaignInsights.recentPublished} انتشار در ۷ روز
                      </StatusToken>
                    }
                    bodyClassName="p-4"
                  >
                    <div className="dashboard-kpi-strip grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                      <MetricTile
                        label="نرخ تحویل"
                        value={`${campaignInsights.deliveryRate}%`}
                        detail={`${campaignInsights.published} منتشر، ${campaignInsights.failed} نیازمند بررسی`}
                        icon={CheckCircle2}
                        tone="success"
                      />
                      <MetricTile
                        label="پوشش رسانه"
                        value={`${campaignInsights.mediaCoverage}%`}
                        detail={`${campaignInsights.withMedia} پست دارای رسانه`}
                        icon={FileImage}
                        tone="info"
                      />
                      <MetricTile
                        label="موفقیت تلاش‌ها"
                        value={`${campaignInsights.attemptSuccessRate}%`}
                        detail={`${selectedAttempts.length} تلاش ثبت‌شده`}
                        icon={Zap}
                        tone="primary"
                      />
                      <MetricTile
                        label="ریسک هفته"
                        value={campaignInsights.recentFailed}
                        detail="خطاهای جدید یا آخرین خطا"
                        icon={AlertTriangle}
                        tone={
                          campaignInsights.recentFailed ? "alert" : "success"
                        }
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-xs leading-5 text-app-muted">
                              بر اساس انتشار، زمان‌بندی، خطا و آخرین تغییر
                              پست‌های همین کمپین.
                            </p>
                          </div>
                          <StatusToken tone="neutral">
                            {campaignInsights.queued} در جریان
                          </StatusToken>
                        </div>
                          ...
                          {campaignInsights.trend.map((point) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="truncate text-[10px] font-bold text-app-muted">
                                  {point.label}
                                </span>
                              </div>
                            );
                      ...
                      <div className="space-y-4">
                        <div className="campaign-glass-card p-4">
                            ...
                            <div className="flex h-full">
                              {campaignInsights.statusMix.map((item) => (
                                <span
                                  key={item.label}
                                  style={{
                                    width: `${percent(item.value, selectedPosts.length)}%`,
                          ...
                          <div className="mt-3 grid grid-cols-2 gap-2">
                            {campaignInsights.statusMix.map((item) => (
                              <div
                                key={item.label}
                                className="rounded-md bg-app-surfaceMuted p-2"
                              >
                        ...
                        <div className="campaign-glass-card p-4">
                          ...
                          {campaignInsights.riskPosts.length === 0 ? (
                            <p className="mt-3 rounded-md bg-emerald-50 p-3 text-xs font-bold leading-5 text-emerald-800">
                              ریسک فعالی برای پست‌های این کمپین دیده نمی‌شود.
                            </p>
                          ) : (
                              ...
                              {campaignInsights.riskPosts.map(
                                ({ post, score }) => (
                                  <Link
                                    ...
                                    className="app-row flex items-center justify-between gap-3 rounded-md border border-app-border bg-app-surfaceMuted p-2 hover:bg-blue-50/60"
                                  >
                                    <span className="min-w-0">
                                      <span className="block truncate text-xs font-black text-app-text">
                                        {post.title}
                                      </span>
                                      <span className="mt-1 block truncate text-[11px] text-app-muted">
                                        {post.last_error ||
                                          `${post.attempt_count} تلاش ثبت‌شده`}
                                      </span>
                                    </span>
                                    <StatusToken
                                      tone={score >= 60 ? "alert" : "warning"}
                                    >
                                      {score}%
                                    </StatusToken>
                                  </Link>
                ...
                <section role="tabpanel">
                  ...
                  >
                    <div className="dashboard-kpi-strip grid gap-3 md:grid-cols-3">
                      <MetricTile
                        label="زمان‌بندی‌شده"
                        value={selectedRow.stats.scheduled}
                        detail="پست دارای تاریخ انتشار"
                        icon={CalendarDays}
                        tone="primary"
                      />
                      <MetricTile
                      ...
                      <MetricTile
                        ...
                        value={Math.max(
                          0,
                          selectedPosts.length - campaignInsights.withMedia,
                        )}
                        detail="پست بدون رسانه متصل"
                        icon={FileImage}
                        tone={
                          selectedPosts.length - campaignInsights.withMedia
                            ? "warning"
                    ...
                    <div className="mt-4 grid gap-2">
                      {selectedTimelinePosts.length === 0 ? (
                        <EmptyState
                          title="برنامه‌ای برای نمایش نیست"
                          description="پست زمان‌بندی‌شده یا منتشرشده‌ای برای این کمپین پیدا نشد."
                          action={
                            <Button
                              href={`/compose?campaignId=${selectedRow.campaign.id}`}
                              variant="secondary"
                        ...
                        selectedTimelinePosts.map((post) => (
                          <Link
                            key={post.id}
                            href={`/compose?postId=${post.id}`}
                            className="campaign-timeline-row app-interactive"
                          >
                            <span className="min-w-0">
                ...
                <section role="tabpanel">
                  <WorkspacePanel
                    ...
                    action={
                      <div className="flex flex-wrap gap-2">
                        <StatusToken tone="neutral">
                          {selectedPosts.length} پست
                        </StatusToken>
                        <Button
                          type="button"
                          size="sm"
                          onClick={openPostAssignment}
                        >
                          <CheckSquare2
                            className="ml-1.5 h-4 w-4"
                            aria-hidden="true"
                    ...
                    <DataTable
                      ...
                      empty={
                        selectedPosts.length === 0 ? (
                          <EmptyState
                            title="هنوز پستی به این کمپین وصل نیست"
                            description="از دکمه افزودن محتوا، پست‌های آماده را به این کمپین وصل کنید."
                            action={
                              <Button
                                type="button"
                                variant="secondary"
                  ...
                  >
                    {selectedAssets.length === 0 ? (
                      <EmptyState
                        ...
                        title="دارایی رسانه‌ای متصل نیست"
                        description="برای حرفه‌ای‌تر شدن کمپین، رسانه‌های مرتبط را به پست‌ها وصل کنید."
                        action={
                          <Button
                            href={`/media?campaignId=${selectedRow.campaign.id}`}
                            variant="secondary"
                      ...
                      <div className="campaign-media-grid">
                        {selectedAssets.slice(0, 12).map((asset) => (
                          <Link
                            key={asset.id}
                            href={`/media?campaignId=${selectedRow.campaign.id}`}
                            className="campaign-media-card app-interactive"
                          >
                            <div className="campaign-media-thumb">
                              {mediaPreviewUrls[asset.id] ? (
                                <img
                                  src={mediaPreviewUrls[asset.id]}
                  ...
                  >
                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                      ...
                      <MetricTile
                        label="ریسک"
                        value={
                          campaignInsights.failed +
                          campaignInsights.recentFailed
                        }
                        detail="خطا و هشدار فعال"
                        icon={AlertTriangle}
                        tone={campaignInsights.failed ? "alert" : "success"}
                      />
                    </div>
                    <div className="mt-4 grid gap-3 lg:grid-cols-2">
                      ...
                      <div className="campaign-glass-card p-4">
                        ...
                        {campaignInsights.riskPosts.length === 0 ? (
                          <p className="mt-3 rounded-md bg-emerald-50 p-3 text-xs font-bold leading-5 text-emerald-800">
                            کمپین برای گزارش مدیریتی آماده است.
                          </p>
                        ) : (
                          <div className="mt-3 grid gap-2">
                            {campaignInsights.riskPosts
                              .slice(0, 4)
                              .map(({ post, score }) => (
                                <Link
                                  key={post.id}
                                  href={`/compose?postId=${post.id}`}
                                  className="app-row flex items-center justify-between gap-3 rounded-md border border-app-border bg-app-surfaceMuted p-2"
                                >
              ...
              {assignmentOpen ? (
                <div
                  ...
                  role="dialog"
                  aria-modal="true"
                  aria-label="افزودن محتوا به کمپین"
                  onClick={() => setAssignmentOpen(false)}
                >
                  <div
                    className="campaign-modal-panel campaign-assignment-panel"
                    onClick={(event) => event.stopPropagation()}
                  >
                    <WorkspacePanel
                      ...
                      action={
                        <StatusToken
                          tone={selectedAssignIds.size ? "primary" : "neutral"}
                        >
                          {selectedAssignIds.size} انتخاب
                        </StatusToken>
                      }
                      bodyClassName="p-3"
                    >
                      <div className="grid gap-2 lg:grid-cols-[minmax(0,1fr)_auto_auto]">
                        <DataSearchField
                          ...
                          placeholder="جست‌وجوی پست برای اتصال"
                        />
                        <Button
                          type="button"
                          variant="secondary"
                          ...
                          disabled={
                            assignablePosts.length === 0 || assigningPosts
                          }
                        >
                          <CheckSquare2
                            className="ml-2 h-4 w-4"
                        ...
                        <Button
                          type="button"
                          size="sm"
                          onClick={assignSelectedPosts}
                          disabled={
                            selectedAssignIds.size === 0 || assigningPosts
                          }
                        >
                      ...
                      <div className="mt-3 max-h-[52vh] overflow-y-auto rounded-md border border-app-border bg-white">
                        {assignablePosts.length === 0 ? (
                          <div className="p-4">
                            <EmptyState
                              title="پست قابل اتصال پیدا نشد"
                              description="همه پست‌های موجود به این کمپین وصل شده‌اند یا نتیجه‌ای برای جست‌وجو وجود ندارد."
                            />
                          </div>
                        ) : (
                          <div className="divide-y divide-app-border">
                            {assignablePosts.map((post) => {
                              ...
                              return (
                                ...
                                >
                                  <span
                                    className={`flex h-5 w-5 shrink-0 items-center justify-center rounded border text-white ${checked ? "border-app-primary bg-app-primary" : "border-app-border bg-white"}`}
                                  >
                                    {checked ? (
                                      <CheckCircle2
                                  ...
                                  <span className="hidden text-[11px] font-bold text-app-muted sm:block">
                                    {formatDateTime(
                                      post.scheduled_at ||
                                        post.published_at ||
                                        post.updated_at,
