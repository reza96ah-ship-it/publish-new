...
function sortQueuePosts(posts: Post[]) {
  return [...posts].sort((first, second) => {
    const priorityDiff =
      (queuePriority[first.status] ?? 9) - (queuePriority[second.status] ?? 9);
...
export default function QueuePage() {
  ...
  const [markingManualPostId, setMarkingManualPostId] = useState<number | null>(
    null,
  );
  ...
  const loadQueue = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      const [response, campaignsResponse, channelData, mediaResponse] =
        await Promise.all([
      ...
      const queuePosts = sortQueuePosts(
        allPosts.filter((post) => queueStatuses.has(post.status)),
      );
      setPosts(queuePosts);
  ...
  async function retryAllFailed() {
    ...
    if (!response.ok) {
      const detail = await readApiError(
        response,
        "بازیابی گروهی صف ناموفق بود",
      );
      setError(detail);
      setRetryingAll(false);
      showToast({
        title: "بازیابی صف ناموفق بود",
        description: detail,
        tone: "alert",
      });
      return;
    ...
    showToast({
      title: "بازیابی صف انجام شد",
      description: `${result.retried_count} پست برای تلاش مجدد آماده شد`,
      tone: "success",
    });
    setRetryingAll(false);
    notifyNotificationsUpdated();
  ...
  async function copyManualCaption(post: Post) {
    ...
    try {
      ...
      showToast({
        title: "کپشن کپی شد",
        description: "حالا می‌توانید آن را در Instagram جای‌گذاری کنید.",
        tone: "success",
      });
    } catch {
      showToast({
        title: "کپی خودکار ناموفق بود",
        description: "کپشن را از پنل بازبین انتخاب و دستی کپی کنید.",
        tone: "warning",
      });
    }
  }
  ...
  async function markManualPublished(post: Post) {
    ...
    const response = await fetch(
      `${apiUrl}/posts/${post.id}/manual-published`,
      {
        method: "POST",
        headers: authHeaders(),
      },
    ...
    if (!response.ok) {
      ...
      showToast({
        title: "ثبت انتشار دستی ناموفق بود",
        description: detail,
        tone: "alert",
      });
      return;
  ...
  const campaignOptions = useMemo(
    () => buildCampaignFilterOptions(posts, campaigns),
    [campaigns, posts],
  );
  ...
  const counts = useMemo(() => {
    return {
      ...
      manual_ready: posts.filter((post) => post.status === "manual_ready")
        .length,
      failed: posts.filter((post) => post.status === "failed").length,
      blockedByReview: posts.filter((post) => approvalBlocksPublishing(post))
        .length,
    };
  }, [posts]);
  ...
  const nextScheduled = useMemo(() => {
    const now = Date.now();
    return posts.find(
      (post) => post.status === "scheduled" && scheduleTime(post) >= now,
    );
  }, [posts]);
  ...
  function postChannelReadiness(post: Post) {
    return normalizeChannels(post.platform).map((channel) => {
      ...
      const mode =
        account?.mode === "instagram_personal_manual"
          ? "دستی"
          : channelCanAutoPublish(account)
            ? "خودکار"
            : channelCanManualPublish(account)
              ? "دستی"
              : "نیازمند تنظیم";
      return {
        label,
    ...
    <AuthGate>
      <AppShell>
        <WorkspacePage className="space-y-4">
            ...
            meta={
              <>
                <StatusToken tone="primary">
                  {posts.length} پست در صف
                </StatusToken>
                <StatusToken tone={counts.failed ? "alert" : "success"}>
                  {counts.failed
                    ? `${counts.failed} خطای فعال`
                    : "بدون خطای فعال"}
                </StatusToken>
                <StatusToken
                  tone={counts.blockedByReview ? "warning" : "success"}
                >
                  {counts.blockedByReview
                    ? `${counts.blockedByReview} منتظر تایید`
                    : "بازبینی پاک"}
                </StatusToken>
                <StatusToken tone={readyChannelCount ? "success" : "warning"}>
                  {readyChannelCount}/{channelAccounts.length || 2} کانال آماده
                </StatusToken>
                {lastUpdatedAt ? (
                  ...
                  <Button
                    ...
                    disabled={
                      ...
                      posts.some(
                        (post) =>
                          post.status === "failed" &&
                          approvalBlocksPublishing(post),
                      )
                    }
                    onClick={retryAllFailed}
                  >
                    <RotateCcw
                      className={`ml-2 h-4 w-4 ${retryingAll ? "animate-spin" : ""}`}
                      aria-hidden="true"
                    />
                    بازیابی همه خطاها
                  </Button>
          ...
          {posts.some(
            (post) =>
              post.status === "failed" && approvalBlocksPublishing(post),
          ) ? (
            <NoticeBanner tone="warning" title="بازیابی گروهی محدود شده است">
              بعضی پست‌های ناموفق هنوز تایید بازبینی ندارند. آن‌ها را از لیست
              محتوا تایید کنید یا جداگانه بررسی کنید.
            </NoticeBanner>
          ) : null}
          {!readyChannelCount ? (
            <NoticeBanner tone="warning" title="هیچ کانال آماده‌ای فعال نیست">
              برای زمان‌بندی و بازیابی صف، ابتدا وضعیت روبیکا یا اینستاگرام را
              در مرکز کانال‌ها کامل کنید.
            </NoticeBanner>
          ) : null}
          ...
          <section className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_340px]">
                ...
                <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_150px_180px] xl:items-center">
                  ...
                  <label className="flex items-center gap-2 rounded-md border border-app-border bg-white px-3 py-2 text-xs font-bold text-app-muted">
                    ...
                    <select
                      value={statusFilter}
                      onChange={(event) =>
                        setStatusFilter(event.target.value as QueueFilter)
                      }
                        ...
                        <ContentOperationCard
                          ...
                          meta={
                            <>
                              ...
                              <p>
                                آخرین تغییر: {formatDateTime(post.updated_at)}
                              </p>
                            </>
                          }
            ...
            <aside className="space-y-3 lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto">
              <div className="space-y-4">
                <WorkspacePanel
                  title="بازبین صف"
                  description="جزئیات و اقدام‌های پست انتخاب‌شده."
                    ...
                    <div>
                      <div className="mb-4 overflow-hidden rounded-md bg-slate-50 ring-1 ring-app-border">
                        {previewUrlForPost(selectedPost) ? (
                          <img
                            src={previewUrlForPost(selectedPost)}
                            alt={
                              primaryMediaForPost(selectedPost)
                                ?.original_filename ?? selectedPost.title
                            }
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <CountdownBadge
                          ...
                          scheduledAt={selectedPost.scheduled_at}
                        />
                        {primaryMediaForPost(selectedPost) ? (
                          <StatusToken tone="success">رسانه آماده</StatusToken>
                        ) : (
                      ...
                      <h2 className="mt-3 font-black text-app-text">
                        {selectedPost.title}
                      </h2>
                      ...
                      <div className="mt-4">
                        <DetailGrid
                          items={[
                            {
                              label: "زمان‌بندی",
                              value: formatDateTime(selectedPost.scheduled_at),
                            },
                            {
                              label: "کمپین",
                              value: campaignLabelForPost(
                                selectedPost,
                                campaigns,
                              ),
                            },
                            ...
                            {
                              label: "تلاش انتشار",
                              value: selectedPost.attempt_count,
                            },
                            {
                              label: "آخرین تغییر",
                              value: formatDateTime(selectedPost.updated_at),
                            },
                            {
                              label: "شناسه پست",
                      ...
                      <div className="mt-4">
                        <p className="mb-3 text-xs font-black text-app-text">
                          مسیر صف
                        </p>
                        <Timeline
                          items={[
                            {
                              title: "ساخت محتوا",
                              description:
                                "رکورد پست در فضای کاری ایجاد شده است.",
                              meta: formatDateTime(selectedPost.created_at),
                              tone: "primary",
                            },
                            {
                              title: selectedPost.scheduled_at
                                ? "ورود به برنامه انتشار"
                                : "آماده‌سازی برای صف",
                              description: selectedPost.scheduled_at
                                ? "زمان انتشار برای این پست ثبت شده است."
                                : "پست منتظر تصمیم بعدی مدیر فضای کاری است.",
                              meta: selectedPost.scheduled_at
                                ? formatDateTime(selectedPost.scheduled_at)
                                : undefined,
                              tone: selectedPost.scheduled_at
                                ? "warning"
                                : "neutral",
                            },
                            {
                              ...
                              description:
                                ...
                                "وضعیت صف برای این پست پایدار است.",
                      ...
                      {approvalBlocksPublishing(selectedPost) ? (
                        <NoticeBanner tone="warning" title="انتشار مسدود است">
                          {
                            approvalConfig(selectedPost.approval_status)
                              .description
                          }
                        </NoticeBanner>
                      ) : null}
                      {selectedPost.status === "manual_ready" ? (
                        <div className="mt-4 space-y-3">
                          <NoticeBanner
                            tone="info"
                            title="آماده انتشار دستی اینستاگرام"
                          >
                            کپشن را کپی کنید، تصویر را در Instagram انتخاب کنید
                            و پس از انتشار، این پست را به عنوان منتشرشده ثبت
                            کنید.
                          </NoticeBanner>
                          <div className="grid gap-2">
                            <Button
                              type="button"
                              onClick={() => copyManualCaption(selectedPost)}
                            >
                              <Clipboard
                            ...
                            <Button
                              type="button"
                              variant="secondary"
                              disabled={markingManualPostId === selectedPost.id}
                              onClick={() => markManualPublished(selectedPost)}
                            >
                              <CheckCircle2
                                className="ml-2 h-4 w-4"
                                aria-hidden="true"
                              />
                              {markingManualPostId === selectedPost.id
                                ? "در حال ثبت"
                                : "ثبت به عنوان منتشرشده"}
                            </Button>
                          </div>
                        </div>
                      ) : null}
                      <div className="mt-4 grid gap-2">
                        ...
                        {selectedPost.status === "failed" ? (
                          <Button
                            type="button"
                            disabled={
                              retryingPostId === selectedPost.id ||
                              approvalBlocksPublishing(selectedPost)
                            }
                            onClick={() => retryPost(selectedPost)}
                          >
                            <RotateCcw
                              className={`ml-2 h-4 w-4 ${retryingPostId === selectedPost.id ? "animate-spin" : ""}`}
                              aria-hidden="true"
                            />
                            {retryingPostId === selectedPost.id
                              ? "در حال ورود به صف"
                        ...
                        {["ready", "scheduled", "manual_ready"].includes(
                          selectedPost.status,
                        ) ? (
                          <Button
                            type="button"
                            variant="ghost"
                            onClick={() => cancelPost(selectedPost)}
                          >
                            لغو از صف
                          </Button>
                        ) : null}
                        <Button href="/logs" variant="secondary">
                          مشاهده لاگ انتشار
                        </Button>
                      </div>
                    </div>
                ...
                {nextScheduled ? (
                  <WorkspacePanel
                    title="انتشار بعدی"
                    description="نزدیک‌ترین پست زمان‌بندی‌شده در صف."
                    bodyClassName="p-3"
                  ...
                  >
                    ...
                    <p className="mt-2 text-sm leading-7 text-app-muted">
                      {formatDateTime(nextScheduled.scheduled_at)}
                    </p>
                    <CountdownBadge
                      status={nextScheduled.status}
                      scheduledAt={nextScheduled.scheduled_at}
                      className="mt-3"
                    />
                  </WorkspacePanel>
                ) : null}
