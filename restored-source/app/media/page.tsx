...
function displayTagLabel(tag: string) {
  ...
  if (normalized.startsWith("source:"))
    return `منبع #${tag.slice("source:".length)}`;
  return tag;
}
...
function isEditedAsset(asset: MediaAsset) {
  return (
    tagList(asset.tags).some((tag) => tag.toLowerCase() === "edited") ||
    /-(edited|variant|rubika-variant|square-variant|portrait-variant|story-variant|landscape-variant)\.png$/i.test(
...
const variantPresets = [
  ...
  {
    label: "عمودی",
    detail: "استوری و محتوای موبایل",
    ratio: 9 / 16,
    sample: "9:16",
  },
  {
    label: "پرتره",
    detail: "پست بلند و معرفی محصول",
    ratio: 4 / 5,
    sample: "4:5",
  },
];
...
export default function MediaPage() {
  ...
  const [confirmDeleteAssetId, setConfirmDeleteAssetId] = useState<
    number | null
  >(null);
  ...
  const loadData = useCallback(async () => {
    ...
    if (mediaResponse.ok) {
      ...
      setSelectedAssetId((current) => {
        if (loadedAssets.some((asset) => String(asset.id) === current))
          return current;
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      showToast({
        title: `${uploadedAssets.length} تصویر آپلود شد`,
        description: uploadFolder.trim()
          ? `پوشه: ${uploadFolder.trim()}`
          : "به کتابخانه رسانه اضافه شد.",
        tone: "success",
      });
      await loadData();
    } catch (err) {
  ...
  async function attachToPost(assetId: number, value: string) {
    ...
    showToast({
      title: "اتصال تصویر ذخیره شد",
      description: postId
        ? "رسانه به پست انتخاب‌شده متصل شد."
        : "رسانه از پست جدا شد.",
      tone: "success",
    });
    await loadData();
  ...
  async function saveMetadata(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/media/${selectedAsset.id}/metadata`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      showToast({
        title: "دسته‌بندی رسانه ذخیره شد",
        description: savedAsset.original_filename,
        tone: "success",
      });
    } catch (err) {
      const nextError =
        err instanceof Error ? err.message : "خطای ذخیره دسته‌بندی";
      setError(nextError);
      showToast({
  ...
  async function deleteAsset(asset: MediaAsset, force = false) {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/media/${asset.id}${force ? "?force=true" : ""}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token()}` },
        },
      ...
      if (response.status === 409) {
        ...
        setError(
          "این رسانه به یک پست متصل است. برای حذف، ابتدا تایید کنید یا آن را از پست جدا کنید.",
        );
        return;
      }
  ...
  async function saveEditedImage(file: File) {
    ...
    try {
      ...
      formData.append(
        "tags",
        mediaTags(editingAsset.tags, "edited", `source:${editingAsset.id}`),
      );
      ...
      setError(nextError);
      showToast({
        title: "ذخیره نسخه جدید ناموفق بود",
        description: nextError,
        tone: "alert",
      });
    } finally {
  ...
  const selectedLinkedPost = selectedAsset?.post_id
    ? (postById.get(selectedAsset.post_id) ?? null)
    : null;
  const selectedLinkedCampaign = selectedLinkedPost
    ? campaignLabelForPost(selectedLinkedPost, campaigns)
    : "";
  ...
  const folders = useMemo(
    () =>
      Array.from(
        new Set(assets.map((asset) => asset.folder.trim()).filter(Boolean)),
      ).sort(),
    [assets],
  );
    ...
    return campaigns
      ...
      .sort(
        (first, second) =>
          ...
          first.campaign.name.localeCompare(second.campaign.name, "fa"),
      );
  }, [assets, campaigns, postById]);
  const mediaSummary = [
    {
      label: "همه رسانه‌ها",
      detail: "دارایی‌های فضای کاری",
      value: "all" as const,
      count: assets.length,
      icon: Images,
    ...
    {
      label: "رسانه آزاد",
      detail: "آماده استفاده در پست",
      value: "unused" as const,
      count: unusedCount,
      icon: FileImage,
      tone: unusedCount ? "text-emerald-700" : "text-slate-500",
    },
    {
      label: "متصل به پست",
      detail: "در حال استفاده",
      value: "attached" as const,
      count: attachedCount,
      icon: Link2,
      tone: attachedCount ? "text-sky-700" : "text-slate-500",
    },
    {
      label: "ویرایش‌شده",
      detail: "خروجی‌های استودیو",
      value: "edited" as const,
      count: editedCount,
      icon: PencilLine,
      tone: editedCount ? "text-violet-700" : "text-slate-500",
    },
    {
      label: "حجم کتابخانه",
      detail: "مصرف فایل‌های رسانه‌ای",
      value: null,
      count: formatSize(totalSizeBytes),
      icon: ImageIcon,
      tone: "text-slate-700",
    },
  ];
  ...
  const filteredAssets = useMemo(() => {
    ...
    return assets.filter((asset) => {
      ...
      const matchesCampaign =
        campaignFilter === "all" ||
        (linkedPost?.campaign_id
          ? campaignFilter === `id:${linkedPost.campaign_id}`
          : false);
      const searchableText =
        `${asset.original_filename} ${asset.content_type} ${asset.folder} ${asset.tags} ${linkedPost?.title ?? ""}`.toLowerCase();
      return (
        matchesFilter &&
        matchesFolder &&
        matchesCampaign &&
  ...
  const selectedPreviewUrl = selectedAsset
    ? mediaPreviewUrls[selectedAsset.id]
    : "";
  useEffect(() => {
  ...
  const selectedVariantReadiness = useMemo(() => {
    ...
    return variantPresets.map((preset) => {
      ...
      return {
        ...
        fitLabel:
          delta <= 0.06 ? "آماده" : delta <= 0.2 ? "کراپ سبک" : "نیازمند نسخه",
      };
    });
  }, [selectedImageSize]);
        ...
        <WorkspacePage>
            ...
            <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-app-muted">
                  تصاویر را جست‌وجو، بررسی و برای استفاده در پست‌های شبکه‌های
                  اجتماعی آماده کنید.
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <StatusToken tone={unusedCount ? "success" : "neutral"}>
                  {unusedCount} رسانه آزاد
                </StatusToken>
                <StatusToken tone="neutral">
                  {formatSize(totalSizeBytes)} حجم کل
                </StatusToken>
                <Button
                  type="button"
                  size="sm"
          ...
          <section className="app-studio-surface grid overflow-hidden rounded-lg sm:grid-cols-2 xl:grid-cols-5">
            {mediaSummary.map((item) => {
              ...
              const content = (
                <>
                  <span
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-slate-50 ${item.tone}`}
                  >
                    <Icon className="h-4 w-4" aria-hidden="true" />
                  </span>
                  <span className="min-w-0">
                    <span className="flex items-baseline gap-2">
                      <span className={`text-lg font-black ${item.tone}`}>
                        {item.count}
                      </span>
                      <span className="truncate text-xs font-bold text-app-text">
                        {item.label}
                      </span>
                    </span>
              ...
              return item.value ? (
                ...
                <div
                  key={item.label}
                  className="flex min-w-0 items-start gap-3 border-b border-app-border p-3 sm:border-l sm:last:border-l-0 xl:border-b-0"
                >
                  {content}
                </div>
              );
          ...
          {showUploader ? (
            <WorkspacePanel
              ...
              action={
                <StatusToken tone={files.length ? "primary" : "neutral"}>
                  {files.length
                ...
                <div className="space-y-4">
                  <label className="app-interactive block cursor-pointer rounded-md border border-dashed border-app-borderStrong bg-app-surfaceMuted px-3 py-4 hover:border-blue-300 hover:bg-blue-50">
                    ...
                    <span className="mt-1 block text-xs text-app-muted">
                      {files.length
                        ? `${files.length} فایل · ${formatSize(files.reduce((total, file) => total + file.size, 0))}`
                        : "تصاویر محصول یا محتوای آماده را یک‌جا انتخاب کنید."}
                    </span>
                  </label>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <Field
                      label="پوشه"
                      hint="برای کمپین، محصول یا فصل محتوایی."
                    >
                      <Input
                        value={uploadFolder}
                        onChange={(event) =>
                    ...
                    <Field label="برچسب‌ها" hint="با ویرگول جدا کنید.">
                      <Input
                        ...
                        placeholder="محصول، بنر، فروش ویژه"
                      />
                    </Field>
                  </div>
                  ...
                  {files.length ? (
                    <div className="flex flex-wrap gap-2">
                      ...
                      {files.length > 6 ? (
                        <Tag tone="primary">+{files.length - 6} فایل دیگر</Tag>
                      ) : null}
                    </div>
                  ) : null}
                ...
                <div className="relative">
                  {selectedFilePreviewUrl ? (
                    <img
                      ...
                      alt="پیش‌نمایش اولین تصویر انتخاب‌شده"
                  ...
                  {files.length > 1 ? (
                    <Tag tone="primary" className="absolute left-2 top-2">
                      +{files.length - 1} تصویر
                    </Tag>
                  ) : null}
                </div>
              </form>
            ...
            <div className="min-w-0 space-y-3">
              ...
              >
                ...
                {folders.length ? (
                  <div className="mb-3 flex flex-wrap gap-2">
                    <button
                      ...
                      className={`app-interactive nashrino-control-radius inline-flex min-h-8 items-center px-3 text-xs font-bold ${folderFilter === "all" ? "bg-app-primary text-white" : "bg-slate-100 text-slate-600 hover:bg-blue-50 hover:text-app-primary"}`}
                    >
                      همه پوشه‌ها
                    </button>
                    {folders.map((folder) => (
                      <button
                        ...
                        className={`app-interactive nashrino-control-radius inline-flex min-h-8 items-center gap-1 px-3 text-xs font-bold ${folderFilter === folder ? "bg-app-primary text-white" : "bg-slate-100 text-slate-600 hover:bg-blue-50 hover:text-app-primary"}`}
                      >
                ...
                {campaignAssetOptions.length ? (
                  <div className="mb-3 rounded-lg border border-app-border bg-app-surfaceMuted p-2.5 sm:p-3">
                    <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                      ...
                      <StatusToken
                        tone={campaignFilter === "all" ? "neutral" : "primary"}
                      >
                        {campaignFilter === "all"
                          ? "همه کمپین‌ها"
                          : "کمپین انتخاب‌شده"}
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        className={`app-interactive nashrino-control-radius inline-flex min-h-8 items-center px-3 text-xs font-bold ${campaignFilter === "all" ? "bg-app-primary text-white" : "bg-white text-slate-600 shadow-hairline hover:bg-blue-50 hover:text-app-primary"}`}
                      >
                        همه کمپین‌ها
                      </button>
                ...
                {!loading && filteredAssets.length > 0 ? (
                  <div
                    className={`${mediaView === "grid" ? "grid gap-2 md:grid-cols-2 2xl:grid-cols-3" : "grid gap-2"} max-h-[68vh] overflow-y-auto pr-1`}
                  >
                    {filteredAssets.map((asset) => {
                      const previewUrl = mediaPreviewUrls[asset.id];
                      const linkedPost = asset.post_id
                        ? postById.get(asset.post_id)
                        : null;
                      const selected = selectedAssetId === String(asset.id);
                      const edited = isEditedAsset(asset);
                      const canEditImage = Boolean(
                        previewUrl && asset.content_type.startsWith("image/"),
                      );
                      ...
                      return (
                        <button
                          ...
                          onDragStart={(event) =>
                            startDraggingAsset(event, asset.id)
                          }
                          onDragEnd={stopDraggingAsset}
                          title={
                            canEditImage
                              ? "یک کلیک برای انتخاب، دوبار کلیک برای ویرایش، کشیدن برای اتصال به پست."
                              : "برای اتصال سریع، رسانه را روی پست مقصد بکشید."
                          }
                        ...
                        >
                          <div
                            className={`relative shrink-0 ${mediaView === "list" ? "w-32 sm:w-44" : ""}`}
                          >
                            {previewUrl ? (
                              <img
                                src={previewUrl}
                            ...
                            <span className="absolute right-2 top-2">
                              <Tag tone={linkedPost ? "primary" : "success"}>
                                {linkedPost ? "در استفاده" : "آزاد"}
                              </Tag>
                            </span>
                            {edited ? (
                            ...
                            {canEditImage ? (
                              <span className="absolute bottom-2 left-2 inline-flex items-center gap-1 rounded bg-white/95 px-2 py-1 text-[10px] font-black text-app-text shadow-hairline ring-1 ring-app-border">
                                <PencilLine
                                  className="h-3 w-3 text-app-primary"
                                  aria-hidden="true"
                                />
                            ...
                            {selected ? (
                              <span className="absolute left-2 top-2 flex h-6 w-6 items-center justify-center rounded-full bg-app-primary text-white shadow-sm">
                                <CheckCircle2
                                  className="h-4 w-4"
                                  aria-hidden="true"
                                />
                          ...
                          <div className="min-w-0 flex-1 p-3">
                            <p
                              className="truncate text-sm font-black text-app-text"
                            ...
                            {tagList(asset.tags).length ? (
                              <div className="mt-2 flex flex-wrap gap-1">
                                {tagList(asset.tags)
                                  .slice(0, 3)
                                  .map((tag) => (
                                    <Tag key={tag}>{displayTagLabel(tag)}</Tag>
                                  ))}
                              </div>
                            ) : null}
              ...
              <WorkspacePanel
                ...
                action={
                  selectedAsset ? (
                    <div className="flex items-center gap-2">
                      <StatusToken
                        tone={selectedLinkedPost ? "primary" : "success"}
                      >
                        {selectedLinkedPost ? "در استفاده" : "آزاد"}
                      </StatusToken>
                      <button
                        type="button"
                ...
                {inspectorTab === "details" && selectedAsset ? (
                  <div className="p-3 sm:p-4">
                    <div className="relative overflow-hidden rounded-md ring-1 ring-app-border">
                      {selectedPreviewUrl ? (
                        <img
                          ...
                          className="aspect-video max-h-64 w-full object-cover lg:max-h-none"
                        />
                      ) : (
                      ...
                      <div className="absolute right-2 top-2 flex flex-wrap gap-1">
                        <Tag
                          tone={
                            isEditedAsset(selectedAsset) ? "primary" : "neutral"
                          }
                        >
                          {isEditedAsset(selectedAsset)
                            ? "نسخه ویرایش‌شده"
                            : "فایل اصلی"}
                        </Tag>
                      </div>
                    ...
                    <div className="mt-4">
                      <DetailGrid
                        items={[
                          ...
                          {
                            label: "نوع",
                            value: selectedAsset.content_type,
                            hint: "فرمت آپلود",
                          },
                          {
                            label: "حجم",
                            value: formatSize(selectedAsset.size_bytes),
                            hint: "اندازه فایل",
                          },
                          {
                            label: "ابعاد",
                          ...
                          {
                            label: "پوشه",
                            value: selectedAsset.folder || "بدون پوشه",
                            hint: "دسته‌بندی کتابخانه",
                          },
                          {
                            label: "برچسب",
                            value: tagList(selectedAsset.tags).length
                              ? tagList(selectedAsset.tags)
                                  .map(displayTagLabel)
                                  .join("، ")
                              : "بدون برچسب",
                            hint: "برچسب‌های رسانه",
                          },
                        ]}
                      />
                    </div>
                      ...
                      {selectedImageSize ? (
                        <div className="mt-3 grid gap-2">
                          {selectedVariantReadiness.map((preset) => (
                            <div
                              key={preset.label}
                              className="rounded-md bg-app-surfaceMuted p-2 shadow-hairline"
                            >
                              <div className="flex items-center justify-between gap-2">
                                <div>
                                  <p className="text-xs font-black text-app-text">
                              ...
                              <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-200">
                                <div
                                  ...
                                  style={{
                                    width: `${Math.max(12, Math.min(100, Math.round((1 - Math.min(preset.delta, 0.5)) * 100)))}%`,
                                  }}
                                />
                              </div>
                            </div>
                        ...
                        <p className="mt-3 rounded-md bg-app-surfaceMuted p-3 text-xs leading-6 text-app-muted">
                          برای تحلیل نسخه‌های خلاقه، پیش‌نمایش تصویر باید در
                          دسترس باشد.
                        </p>
                      )}
                    </div>
                    ...
                    <form
                      ...
                      className="mt-4 rounded-md bg-app-surfaceMuted p-3 shadow-hairline"
                    >
                      <div className="flex items-center gap-2">
                        <Folder
                          className="h-4 w-4 text-app-primary"
                      ...
                      <div className="mt-3 grid gap-3">
                        <Field label="پوشه">
                          <Input
                            ...
                            onChange={(event) =>
                              setMetadataFolder(event.target.value)
                            }
                            placeholder="مثلاً محصولات تابستانی"
                          />
                        </Field>
                        <Field
                          label="برچسب‌ها"
                          hint="برای جست‌وجوی سریع‌تر با ویرگول جدا کنید."
                        >
                          <Input
                            value={metadataTags}
                            onChange={(event) =>
                            ...
                            placeholder="محصول، استوری، فروش ویژه"
                          />
                        </Field>
                      </div>
                      {tagList(metadataTags).length ? (
                        <div className="mt-3 flex flex-wrap gap-1">
                          {tagList(metadataTags).map((tag) => (
                            <Tag key={tag}>
                              <Hash
                    ...
                    <div className="mt-4 rounded-md bg-app-surfaceMuted p-3 shadow-hairline">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <StatusToken
                          tone={selectedLinkedPost ? "primary" : "success"}
                        >
                          {selectedLinkedPost ? "۱ مصرف فعال" : "بدون مصرف"}
                        </StatusToken>
                      </div>
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        <Tag tone={selectedLinkedPost ? "primary" : "success"}>
                          {selectedLinkedPost ? "متصل به پست" : "بدون اتصال"}
                        </Tag>
                        {selectedLinkedPost ? (
                          <StatusBadge status={selectedLinkedPost.status} />
                        ) : null}
                      </div>
                      {selectedLinkedPost ? (
                        <div className="mt-3 rounded-md bg-white p-3 text-xs leading-6 shadow-hairline">
                          ...
                          <p className="mt-1 text-app-muted">
                            کمپین: {selectedLinkedCampaign}
                          </p>
                          <p className="mt-1 text-app-muted">
                            این رسانه در پست #{selectedLinkedPost.id} استفاده
                            می‌شود. حذف آن بعد از تایید، رسانه را از کتابخانه
                            حذف می‌کند و پست بدون رسانه می‌ماند.
                          </p>
                        </div>
                      ) : (
                        <p className="mt-3 rounded-md bg-white p-3 text-xs leading-6 text-app-muted shadow-hairline">
                          این فایل در هیچ پستی استفاده نشده و برای استفاده مجدد
                          در composer یا کمپین‌های بعدی آماده است.
                        </p>
                      )}
                    </div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      {selectedLinkedPost ? (
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            void attachToPost(selectedAsset.id, "")
                          }
                        >
                          <XCircle
                            className="ml-2 h-4 w-4"
                      ...
                      {selectedLinkedPost ? (
                        <Button
                          href={`/compose?postId=${selectedLinkedPost.id}`}
                          variant="secondary"
                          size="sm"
                        >
                          باز کردن پست
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-[11px] leading-5 text-rose-700">
                            {selectedLinkedPost
                              ? "این رسانه در یک پست استفاده شده است؛ حذف فقط بعد از تایید دوم انجام می‌شود."
                              : "این رسانه استفاده فعالی ندارد و می‌تواند از کتابخانه حذف شود."}
                          </p>
                        </div>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        ...
                        confirmDeleteAssetId !== selectedAsset.id ? (
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              setConfirmDeleteAssetId(selectedAsset.id)
                            }
                          >
                            <Trash2
                              className="ml-2 h-4 w-4"
                  ...
                  <div className="p-4">
                    <div className="mb-3 flex items-center justify-between gap-2">
                      ...
                      <StatusToken
                        tone={draggingAssetId ? "primary" : "neutral"}
                      >
                        {draggingAssetId ? "مقصد را انتخاب کنید" : "اتصال سریع"}
                      </StatusToken>
                    </div>
                    {posts.length ? (
                        ...
                        {posts.slice(0, 8).map((post) => {
                          ...
                          return (
                            <button
                              ...
                              onDragOver={(event) =>
                                allowPostDrop(event, post.id)
                              }
                              onDragLeave={() => setDropTargetPostId(null)}
                              onDrop={(event) =>
                                dropAssetOnPost(event, post.id)
                              }
                              className={`app-interactive flex min-w-0 items-center justify-between gap-3 rounded-md px-3 py-2 text-right shadow-hairline disabled:cursor-not-allowed disabled:opacity-55 ${
                                isDropTarget
                                  ? "bg-blue-50 ring-2 ring-app-primary"
                      ...
                      <EmptyState
                        title="هنوز پستی ساخته نشده است"
                        description="برای استفاده از اتصال سریع، ابتدا یک پست بسازید."
                      />
                    )}
                    {selectedAsset ? (
                      <label className="mt-4 block text-xs font-bold text-app-muted">
                        ...
                        <select
                          value={
                            selectedAsset.post_id
                              ? String(selectedAsset.post_id)
                              : ""
                          }
                          onChange={(event) =>
                            void attachToPost(
