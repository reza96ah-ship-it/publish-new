...
function normalizeStore(data: Partial<StoreForm> = {}): StoreForm {
  return {
    ...
    logo_asset_id:
      typeof data.logo_asset_id === "number" ? data.logo_asset_id : null,
    avatar_asset_id:
      typeof data.avatar_asset_id === "number" ? data.avatar_asset_id : null,
    brand_primary_color: isHexColor(data.brand_primary_color ?? "")
      ? (data.brand_primary_color ?? "#0F766E")
      : "#0F766E",
    brand_accent_color: isHexColor(data.brand_accent_color ?? "")
      ? (data.brand_accent_color ?? "#2563EB")
      : "#2563EB",
...
function buildReadiness(form: StoreForm): ReadinessItem[] {
  return [
    ...
    },
    {
      ...
      detail:
        "برای تبدیل پیش‌نمایش‌ها و گزارش‌ها از حالت متنی به هویت واقعی برند.",
...
function defaultCount(form: StoreForm) {
  return [
    form.logo_asset_id,
    form.avatar_asset_id,
    form.default_hashtags,
    form.caption_footer,
    form.description,
    form.brand_voice,
    form.default_cta,
    form.content_guidelines,
...
function ReadinessRow({ item }: { item: ReadinessItem }) {
  ...
  return (
    <div className="flex items-start gap-3 border-b border-app-border py-3 first:pt-0 last:border-0 last:pb-0">
      <Icon
        className={`mt-0.5 h-5 w-5 shrink-0 ${item.done ? "text-emerald-700" : item.required ? "text-amber-700" : "text-slate-400"}`}
        aria-hidden="true"
      />
      <div>
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        {selectedId ? (
          <button
            type="button"
            onClick={onClear}
            className="rounded-md p-1.5 text-slate-400 transition hover:bg-slate-50 hover:text-rose-600"
        ...
        <div className="min-w-0 space-y-2">
          ...
          {selectedAsset ? (
            <p className="truncate text-[11px] text-app-muted">
              {selectedAsset.folder || "بدون پوشه"} ·{" "}
              {selectedAsset.content_type}
            </p>
          ) : (
            <p className="text-[11px] leading-5 text-app-muted">
              {assets.length
                ? "یا یکی از تصاویر موجود را انتخاب کنید."
                : "هنوز تصویر آماده‌ای در کتابخانه نیست."}
            </p>
          )}

          {selectedId && !previewUrls[selectedId] ? (
            <p className="text-[11px] text-amber-700">
              پیش‌نمایش این دارایی در دسترس نیست.
            </p>
          ) : null}
        </div>
      </div>
...
export default function StorePage() {
  ...
  >({});
  const [uploadingAsset, setUploadingAsset] = useState<BrandAssetKind | null>(
    null,
  );
  ...
  useEffect(() => {
    setBrandPrimaryColorDraft(
      isHexColor(form.brand_primary_color)
  ...
  useEffect(() => {
    setBrandAccentColorDraft(
      isHexColor(form.brand_accent_color) ? form.brand_accent_color : "#2563EB",
    );
  ...
  const requiredReady = readinessItems
    .filter((item) => item.required)
    .every((item) => item.done);
  const imageAssets = useMemo(
    () =>
      mediaAssets.filter((asset) => asset.content_type.startsWith("image/")),
    [mediaAssets],
  );
  const logoUrl = form.logo_asset_id
    ? mediaPreviewUrls[form.logo_asset_id]
    : "";
  const avatarUrl = form.avatar_asset_id
    ? mediaPreviewUrls[form.avatar_asset_id]
    : "";
  ...
  function updateField(
    ...
    value: StoreForm[keyof StoreForm],
  ) {
    setMessage("");
    setForm((current) => ({ ...current, [field]: value }));
  }

  function commitBrandColor(
    field: "brand_primary_color" | "brand_accent_color",
    value: string,
  ) {
  ...
  async function uploadBrandAsset(
    ...
    try {
      ...
      formData.append(
        "tags",
        kind === "logo_asset_id" ? "brand,logo" : "brand,avatar",
      );
      ...
      showToast({
        title: "دارایی برند آپلود شد",
        description:
          kind === "logo_asset_id"
            ? "لوگوی برند انتخاب شد."
            : "آواتار برند انتخاب شد.",
        tone: "success",
      });
    } catch (err) {
      const nextError =
        err instanceof Error ? err.message : "خطای آپلود دارایی برند";
      setError(nextError);
      showToast({
  ...
  async function saveStore(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      showToast({
        title: "پروفایل فروشگاه ذخیره شد",
        description: "متن‌های پایه از این لحظه در composer قابل استفاده‌اند.",
        tone: "success",
      });
      notifyWorkspaceUpdated();
    } catch (err) {
      const nextError =
        err instanceof Error ? err.message : "خطای ذخیره اطلاعات";
      setError(nextError);
      showToast({
        title: "ذخیره پروفایل ناموفق بود",
        description: nextError,
        tone: "alert",
      });
    } finally {
      ...
      <AppShell>
        <WorkspacePage>
            ...
            <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-app-muted">
                  هویت فروشگاه و متن‌های ثابت را برای تولید محتوای منظم نگه
                  دارید.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <StatusToken tone={requiredReady ? "success" : "warning"}>
                  {requiredReady ? "حداقل آماده" : "نیازمند تکمیل"}
                </StatusToken>
                <StatusToken tone={saving || dirty ? "warning" : "success"}>
                  {saving
                    ? "در حال ذخیره"
                    : dirty
                      ? "تغییرات ذخیره نشده"
          ...
          <section className="grid overflow-hidden rounded-md border border-app-border bg-white sm:grid-cols-3">
            {[
              {
                label: "آمادگی پروفایل",
                value: `${score}%`,
                detail: "نام و منطقه زمانی پایه‌های ضروری‌اند",
                icon: StoreIcon,
                tone: requiredReady ? "text-emerald-700" : "text-amber-700",
              },
              {
                label: "کیت برند",
                value: `${defaultCount(form)}/8`,
                detail: "لوگو، آواتار، لحن، CTA و قوانین",
                icon: Palette,
                tone: "text-app-primary",
              },
              {
                label: "وضعیت ویرایش",
                value: dirty ? "ذخیره نشده" : "به‌روز",
                detail: dirty
                  ? "نسخه جدید را ثبت کنید"
                  : "آخرین تغییرات ثبت شده است",
                icon: Save,
                tone: dirty ? "text-amber-700" : "text-emerald-700",
              },
            ].map((metric) => {
              ...
              return (
                <div
                  key={metric.label}
                  className="flex min-w-0 items-start gap-3 border-b border-app-border p-3 sm:border-b-0 sm:border-l sm:last:border-l-0"
                >
                  <span
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-slate-50 ${metric.tone}`}
                  >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 truncate text-base font-black text-app-text">
                      {metric.value}
                    </p>
                    <p className="truncate text-[11px] text-app-muted">
                      {metric.detail}
              ...
              <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_340px]">
                <div className="space-y-3 lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto lg:pr-1">
                  <WorkspacePanel
                    ...
                    action={
                      <Tag tone={requiredReady ? "success" : "warning"}>
                        {requiredReady ? "اطلاعات پایه آماده" : "نیازمند تکمیل"}
                      </Tag>
                    }
                    bodyClassName="p-0"
                  >
                    <div className="grid gap-3 p-3 sm:p-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(240px,0.65fr)]">
                      <Field
                        label="نام فروشگاه"
                        required
                        hint="نامی که در workspace و پیش‌نمایش کپشن نمایش داده می‌شود."
                      >
                        <Input
                          value={form.name}
                          onChange={(event) =>
                          ...
                          placeholder="مثلاً فروشگاه سپهر"
                          required
                        />
                      </Field>
                      ...
                      <Field
                        label="دسته‌بندی فعالیت"
                        hint="برای دسته‌بندی محتوا و ساخت کمپین‌های منظم‌تر."
                      >
                        <Input
                          value={form.category}
                          onChange={(event) =>
                          ...
                          placeholder="مثلاً پوشاک یا محصولات آرایشی"
                        />
                      </Field>
                    </div>
                    ...
                    <div className="grid gap-3 border-t border-app-border bg-slate-50/70 p-3 sm:p-4 md:grid-cols-2">
                      <Field
                        label="شماره تماس"
                        hint="در صورت نیاز برای CTA و اطلاعات تماس مشتری استفاده می‌شود."
                      >
                        <Input
                          value={form.phone}
                          onChange={(event) =>
                      ...
                      <Field
                        label="منطقه زمانی"
                        required
                        hint="مبنای زمان‌بندی صف انتشار برای این workspace."
                      >
                        <div className="relative">
                          <Input
                            value={form.timezone}
                            readOnly
                            className="bg-white pl-10 text-left text-slate-600"
                            dir="ltr"
                            required
                          />
                          <Clock3
                  ...
                  <WorkspacePanel
                    ...
                    action={
                      <Tag
                        tone={
                          form.logo_asset_id || form.avatar_asset_id
                            ? "success"
                            : "warning"
                        }
                      >
                  ...
                  >
                    <div className="grid gap-4 lg:grid-cols-2">
                      <BrandAssetPicker
                        ...
                        onUpload={(file) =>
                          void uploadBrandAsset("logo_asset_id", file)
                        }
                        onSelect={(assetId) =>
                          updateField("logo_asset_id", assetId)
                        }
                      ...
                      <BrandAssetPicker
                        ...
                        onUpload={(file) =>
                          void uploadBrandAsset("avatar_asset_id", file)
                        }
                        onSelect={(assetId) =>
                          updateField("avatar_asset_id", assetId)
                        }
                  ...
                  <WorkspacePanel
                    ...
                    action={
                      <Tag
                        tone={
                          form.brand_voice || form.default_cta
                            ? "success"
                            : "warning"
                        }
                      >
                  ...
                  >
                    <div className="grid gap-5 lg:grid-cols-2">
                      <Field
                        label="رنگ اصلی برند"
                        hint="در آواتار، پیش‌نمایش و وضعیت‌های برند استفاده می‌شود."
                      >
                        <div className="grid min-w-0 grid-cols-[64px_minmax(0,1fr)] items-center gap-3">
                          <Input
                            type="color"
                            value={
                              isHexColor(brandPrimaryColorDraft)
                                ? brandPrimaryColorDraft
                                : "#0F766E"
                            }
                            onInput={(event) =>
                              setBrandPrimaryColorDraft(
                            ...
                            onChange={(event) =>
                              setBrandPrimaryColorDraft(event.target.value)
                            }
                            onBlur={(event) =>
                              commitBrandColor(
                                "brand_primary_color",
                      ...
                      <Field
                        label="رنگ مکمل برند"
                        hint="برای تاکیدهای ثانویه، CTA و گزارش‌های آینده."
                      >
                        <div className="grid min-w-0 grid-cols-[64px_minmax(0,1fr)] items-center gap-3">
                          <Input
                            type="color"
                            value={
                              isHexColor(brandAccentColorDraft)
                                ? brandAccentColorDraft
                                : "#2563EB"
                            }
                            onInput={(event) =>
                              setBrandAccentColorDraft(
                            ...
                            onChange={(event) =>
                              setBrandAccentColorDraft(event.target.value)
                            }
                            onBlur={(event) =>
                              commitBrandColor(
                                "brand_accent_color",
                      ...
                      <Field
                        label="لحن برند"
                        hint="مثلاً صمیمی، مطمئن، اقتصادی، لوکس یا آموزشی."
                      >
                        <Textarea
                      ...
                      <Field
                        label="دعوت به اقدام پیش‌فرض"
                        hint="CTA کوتاه که در شروع سریع composer و پیش‌نمایش استفاده می‌شود."
                      >
                        <Textarea
                          value={form.default_cta}
                          onChange={(event) =>
                            updateField("default_cta", event.target.value)
                          }
                      ...
                      <div className="lg:col-span-2">
                        <Field
                          label="قوانین محتوایی برند"
                          hint="مواردی که کپشن‌ها باید رعایت کنند یا از آن دوری کنند."
                        >
                          <Textarea
                  ...
                  <WorkspacePanel
                    title="پیش‌فرض‌های انتشار"
                    description="متن‌های تکرارشونده را یک‌بار تنظیم کنید تا composer شروع سریع‌تری داشته باشد."
                  >
                    <div className="grid gap-5 lg:grid-cols-2">
                      <Field
                        label="توضیحات کوتاه فروشگاه"
                        hint="یک توضیح کوتاه که شخصیت برند و پیشنهاد اصلی را مشخص کند."
                      >
                        <Textarea
                      ...
                      <Field
                        label="هشتگ‌های پیش‌فرض"
                        hint="در هر خط یا با فاصله بنویسید."
                      >
                        <Textarea
                          value={form.default_hashtags}
                          onChange={(event) =>
                            updateField("default_hashtags", event.target.value)
                          }
                      ...
                      <Field
                        label="متن پایانی کپشن"
                        hint="دعوت به اقدام ثابت مثل سفارش، تماس یا مراجعه حضوری."
                      >
                        <Textarea
                          value={form.caption_footer}
                          onChange={(event) =>
                            updateField("caption_footer", event.target.value)
                          }
                      ...
                      <div className="lg:col-span-2">
                        <NoticeBanner>
                          composer از این اطلاعات برای شروع سریع‌تر کپشن‌ها
                          استفاده می‌کند.
                        </NoticeBanner>
                      </div>
                    </div>
                  ...
                  <div className="sticky bottom-4 z-10 flex flex-col gap-3 rounded-md border border-app-border bg-white/95 p-3 shadow-lg shadow-slate-200/60 backdrop-blur md:flex-row md:items-center md:justify-between">
                    <div>
                      <p className="text-sm font-black text-app-text">
                        {dirty
                          ? "تغییرات آماده ذخیره است"
                          : "پروفایل فروشگاه به‌روز است"}
                      </p>
                      <p className="mt-1 text-xs text-app-muted">
                        {dirty
                          ? "برای استفاده در composer، نسخه جدید را ثبت کنید."
                          : "هر تغییر جدید در این نوار مشخص می‌شود."}
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <Button
                ...
                <aside className="hidden space-y-3 lg:sticky lg:top-24 lg:block lg:max-h-[calc(100vh-7rem)] lg:self-start lg:overflow-y-auto">
                  <WorkspacePanel
                    title="چک‌لیست آماده‌سازی"
                    description="برای یک workspace قابل اتکا، این موارد را کامل نگه دارید."
                  >
                  ...
                  <WorkspacePanel
                    title="پیش‌نمایش کپشن پایه"
                    description="خروجی پایه‌ای که در کپشن‌ها تکرار می‌شود."
                  >
                    <div className="rounded-md border border-app-border bg-slate-50 p-4">
                      <div className="mb-4 flex items-center gap-3 border-b border-app-border pb-3">
                        <WorkspaceAvatar
                          name={form.name || "نام فروشگاه"}
                          color={form.brand_primary_color}
                          imageUrl={avatarUrl || logoUrl}
                        />
                        <div className="min-w-0">
                          <p className="truncate text-sm font-black text-app-text">
                            {form.name || "نام فروشگاه"}
                          </p>
                    ...
                    <div className="mt-4 grid gap-3 text-xs text-app-muted">
                      ...
                      <p className="flex items-center gap-2">
                        ...
                        {form.default_hashtags
                          ? "هشتگ پیش‌فرض آماده است"
                          : "هشتگ پیش‌فرض هنوز خالی است"}
                      </p>
                    </div>
                  </WorkspacePanel>
                  ...
                  <WorkspacePanel
                    title="مرحله بعدی"
                    description="بعد از هویت برند، کانال‌های انتشار را کامل کنید."
                  >
                    <Button href="/channels" className="w-full">
                      باز کردن مرکز کانال‌ها
                    </Button>
                  </WorkspacePanel>
                </aside>
              </div>
