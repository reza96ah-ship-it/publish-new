...
import { NInspectorDrawer } from "@/components/nahrino-ui";
...
import {
  ...
  campaignColorForPost,
  campaignKeyForPost,
  campaignLabelForPost,
  loadCampaigns,
  type Campaign,
} from "../../lib/campaigns";
...
function buildWeekDays(anchorIso: string) {
  ...
  return Array.from({ length: 7 }, (_, index) =>
    makeCalendarDay(addDays(start, index)),
  );
}
...
export default function CalendarPage() {
  ...
  const hasDayConflicts = useCallback((dayPosts: Post[]) => {
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      const gap = minutesBetween(
        sorted[i].scheduled_at,
        sorted[i + 1].scheduled_at,
      );
  ...
  const [rubikaSettings, setRubikaSettings] = useState<RubikaSettings | null>(
    null,
  );
  ...
  const [quickPreviewPostId, setQuickPreviewPostId] = useState<number | null>(
    null,
  );
  ...
  const [reschedulingPostId, setReschedulingPostId] = useState<number | null>(
    null,
  );
  const [rescheduleDraftPostId, setRescheduleDraftPostId] = useState<
    number | null
  >(null);
  ...
  const loadPosts = useCallback(async (preservePlannerState = false) => {
    ...
    const [response, campaignsResponse, mediaResponse, rubikaResponse] =
      await Promise.all([
    ...
    if (!preservePlannerState) {
      setMonthAnchor(
        upcoming?.scheduled_at ??
          sorted[0]?.scheduled_at ??
          new Date().toISOString(),
      );
      setSelectedDayKey(
        upcoming?.scheduled_at
          ? jalaliDateKey(upcoming.scheduled_at)
          : sorted[0]?.scheduled_at
            ? jalaliDateKey(sorted[0].scheduled_at)
            : jalaliDateKey(new Date().toISOString()),
      );
  ...
  useEffect(() => {
    loadPosts().catch((err) => {
      setError(
        err instanceof Error ? err.message : "خطا در دریافت تقویم انتشار",
      );
      setLoading(false);
    });
  }, [loadPosts]);
  ...
  useEffect(() => {
    const imageAssets = assets.filter(
      (asset) => asset.post_id && asset.content_type.startsWith("image/"),
    );
    ...
    async function loadPreviews() {
      ...
      if (!cancelled)
        setMediaPreviewUrls(
          Object.fromEntries(
            entries.filter(Boolean) as Array<[number, string]>,
          ),
        );
    }
  ...
  const calendarPosts = useMemo(
    () => sortByScheduleAsc(posts.filter(isCalendarPost)),
    [posts],
  );
  const filteredPosts = useMemo(() => {
    ...
    return calendarPosts
      ...
      .filter(
        (post) =>
          campaignFilter === "all" ||
          campaignKeyForPost(post) === campaignFilter,
      )
  ...
  const campaignOptions = useMemo(() => {
    ...
    if (
      !presetCampaignId ||
      options.some((option) => option.value === `id:${presetCampaignId}`)
    )
      return options;
    const routedCampaign = campaigns.find(
      (campaign) => String(campaign.id) === presetCampaignId,
    );
    return routedCampaign
      ? [
          {
            value: `id:${routedCampaign.id}`,
            label: routedCampaign.name,
            color: routedCampaign.color,
            count: 0,
          },
          ...options,
        ]
  ...
  const selectedCampaignOption = useMemo(
    () =>
      campaignOptions.find((option) => option.value === campaignFilter) ?? null,
    [campaignFilter, campaignOptions],
  );
  const selectedCampaignIdForRoute = selectedCampaignOption?.value.startsWith(
    "id:",
  )
    ? selectedCampaignOption.value.replace("id:", "")
    : "";

  useEffect(() => {
    if (!presetCampaignId) return;
    const nextValue = `id:${presetCampaignId}`;
    if (campaignOptions.some((option) => option.value === nextValue))
  ...
  const activeWeekDays = useMemo(
    () => buildWeekDays(monthAnchor),
    [monthAnchor],
  );
  const activeRangeDays = viewMode === "week" ? activeWeekDays : monthDays;
  const activeRangeDayKeys = useMemo(
    () => new Set(activeRangeDays.map((day) => day.key)),
    [activeRangeDays],
  );
  ...
  const rescheduleDraftPost = useMemo(() => {
    ...
    return (
      calendarPosts.find((post) => post.id === rescheduleDraftPostId) ?? null
    );
  }, [calendarPosts, rescheduleDraftPostId]);
  const assetByPostId = useMemo(() => {
    const map = new Map<number, MediaAsset>();
    for (const asset of assets) {
      if (asset.post_id && !map.has(asset.post_id))
        map.set(asset.post_id, asset);
    }
    return map;
  }, [assets]);
  ...
  useEffect(() => {
    setTodayKey(jalaliDateKey(new Date().toISOString()));
    setNow(Date.now());
  }, []);
  ...
  const attentionPosts = calendarPosts.filter((post) => {
    ...
    return (
      post.status === "failed" ||
      (post.status === "scheduled" && time !== null && time < now)
    );
  });
  const monthPostCount = filteredPosts.filter(
    (post) =>
      postsByDay.has(jalaliDateKey(post.scheduled_at)) &&
      monthDays.some((day) => day.key === jalaliDateKey(post.scheduled_at)),
  ).length;
  const activeDayKey =
    selectedDayKey ??
    (selectedPost?.scheduled_at
      ? jalaliDateKey(selectedPost.scheduled_at)
      : todayKey);
  const selectedDayPosts = useMemo(
    () => (activeDayKey ? (postsByDay.get(activeDayKey) ?? []) : []),
    [activeDayKey, postsByDay],
  );
  const selectedDay =
    [...monthDays, ...activeWeekDays].find((day) => day.key === activeDayKey) ??
    null;
  const selectedDayValue = selectedDay?.date ?? monthAnchor;
  const selectedDayLabel = formatJalaliDate(selectedDayValue);
  const publishedCount = calendarPosts.filter(
    (post) => post.status === "published",
  ).length;
  const scheduledCount = calendarPosts.filter(
    (post) => post.status === "scheduled",
  ).length;
  const failedCount = calendarPosts.filter(
    (post) => post.status === "failed",
  ).length;
  const publishingCount = calendarPosts.filter(
    (post) => post.status === "publishing",
  ).length;
  const plannerHealthTone = attentionPosts.length
    ? "alert"
    : failedCount
      ? "warning"
      : "success";
  const plannerHealthLabel = attentionPosts.length
    ? `${attentionPosts.length} اقدام فوری`
    : failedCount
      ? `${failedCount} خطا`
      : "برنامه پایدار";
  const currentRangeLabel =
    viewMode === "week"
      ? dayRangeLabel(activeWeekDays)
      : viewMode === "list"
        ? "فهرست برنامه"
        : formatJalaliMonth(monthAnchor);
  const visibleRangeCount = filteredPosts.filter(
    (post) =>
      post.scheduled_at &&
      activeRangeDayKeys.has(jalaliDateKey(post.scheduled_at)),
  ).length;
  ...
  const calendarCellHeight =
    densityMode === "compact" ? "min-h-24" : "min-h-36";

  const rescheduleDraftIso = useMemo(() => {
    if (!rescheduleDraftPost || !rescheduleDraftDay) return null;
    return jalaliDateToIsoAtTime(
      rescheduleDraftDay,
      rescheduleDraftHour,
      rescheduleDraftMinute,
      scheduleTimezone,
    );
  }, [
    rescheduleDraftDay,
  ...
  const rescheduleDraftConflicts = useMemo(() => {
    ...
    return calendarPosts.filter((post) => {
      if (post.id === rescheduleDraftPost.id || !post.scheduled_at)
        return false;
  ...
  const selectedCampaignWorkload = useMemo(() => {
    ...
    const rangePosts = calendarPosts
      .filter(
        (post) => campaignKeyForPost(post) === selectedCampaignOption.value,
      )
      .filter(
        (post) =>
          post.scheduled_at &&
    ...
    return {
      ...
      scheduled: rangePosts.filter((post) => post.status === "scheduled")
        .length,
      publishing: rangePosts.filter((post) => post.status === "publishing")
        .length,
  ...
  const selectedDayInsights = useMemo(() => {
    ...
    sortedDayPosts.forEach((post, index) => {
      ...
      const gap = minutesBetween(
        post.scheduled_at,
        nextPostForDay.scheduled_at,
      );
      if (gap !== null && gap < 90)
        conflicts.push({
          first: post,
          second: nextPostForDay,
          gap: Math.round(gap),
        });
    });
    const missingMedia = sortedDayPosts.filter(
      (post) => !assetByPostId.has(post.id),
    );
    const failed = sortedDayPosts.filter(
      (post) => post.status === "failed" || post.last_error,
    );
    const publishablePosts = sortedDayPosts.filter((post) =>
      ["scheduled", "publishing"].includes(post.status),
    );
    const rubikaBlocked =
      publishablePosts.length > 0 && !isRubikaConnected(rubikaSettings);
    const busyHours = new Set(
      sortedDayPosts.map(
        (post) =>
          getJalaliPickerParts(post.scheduled_at, scheduleTimezone).hour,
      ),
    );
  ...
  const selectedDayNextAction = useMemo(() => {
    if (selectedDayInsights.rubikaBlocked)
      return {
        tone: "alert" as const,
        title: "اتصال کانال را بررسی کنید",
        detail: rubikaStatusLabel(rubikaSettings),
      };
    if (selectedDayInsights.failed.length)
      return {
        tone: "alert" as const,
        title: "پست خطادار را اصلاح کنید",
        detail: `${selectedDayInsights.failed.length} پست نیازمند بررسی است`,
      };
    if (selectedDayInsights.conflicts.length)
      return {
        tone: "warning" as const,
        title: "فاصله انتشار را بهتر کنید",
        detail: `${selectedDayInsights.conflicts.length} تداخل نزدیک پیدا شد`,
      };
    if (selectedDayInsights.missingMedia.length)
      return {
        tone: "warning" as const,
        title: "رسانه پست‌ها را کامل کنید",
        detail: `${selectedDayInsights.missingMedia.length} پست بدون رسانه است`,
      };
    if (!selectedDayPosts.length)
      return {
        tone: "primary" as const,
        title: "روز خالی آماده برنامه‌ریزی است",
        detail: "از زمان‌های پیشنهادی یک پست تازه بسازید",
      };
    return {
      tone: "success" as const,
      title: "روز آماده انتشار است",
      detail: "فاصله‌بندی و وضعیت پست‌ها پایدار است",
    };
  ...
  const quickPreviewReadiness = useMemo(() => {
    ...
    const dayConflicts = quickPreviewPost.scheduled_at
      ? calendarPosts.filter((post) => {
          if (post.id === quickPreviewPost.id || !post.scheduled_at)
            return false;
          if (
            jalaliDateKey(post.scheduled_at) !==
            jalaliDateKey(quickPreviewPost.scheduled_at)
          )
            return false;
          const gap = minutesBetween(
            quickPreviewPost.scheduled_at,
            post.scheduled_at,
          );
          return gap !== null && gap < 90;
        })
      : [];
    const publishable = ["scheduled", "publishing"].includes(
      quickPreviewPost.status,
    );
    return [
      {
        label: "رسانه",
        value: assetByPostId.has(quickPreviewPost.id)
          ? "آماده"
          : "نیازمند رسانه",
        tone: assetByPostId.has(quickPreviewPost.id)
          ? ("success" as const)
          : ("warning" as const),
      },
      {
        label: "کانال",
        value:
          !publishable || isRubikaConnected(rubikaSettings)
            ? "متصل"
            : "نیازمند اتصال",
        tone:
          !publishable || isRubikaConnected(rubikaSettings)
      ...
      {
        label: "فاصله زمانی",
        value: dayConflicts.length ? `${dayConflicts.length} تداخل` : "ایمن",
        tone: dayConflicts.length ? ("warning" as const) : ("success" as const),
      },
      {
        label: "خطا",
        value: quickPreviewPost.last_error ? "نیازمند بررسی" : "بدون خطا",
        tone: quickPreviewPost.last_error
          ? ("alert" as const)
          : ("success" as const),
      },
    ];
  ...
  function focusSelectedDayAgenda(dayKey: string) {
    ...
    window.setTimeout(() => {
      setAgendaPulseKey((current) =>
        current.startsWith(`${dayKey}-`) ? "" : current,
      );
    }, 1000);
  }
      ...
      calendarPosts
        .filter(
          (post) =>
            ...
            post.scheduled_at &&
            jalaliDateKey(post.scheduled_at) === dayKey,
        )
        .map(
          (post) =>
            getJalaliPickerParts(post.scheduled_at, scheduleTimezone).hour,
  ...
  function openRescheduleDraft(
    post: Post,
    nextDay = post.scheduled_at ?? selectedDayValue,
  ) {
    if (!canReschedule(post)) {
      showToast({
        title: "تغییر زمان فعال نیست",
        description: "فقط پست‌های زمان‌بندی‌شده قابل جابجایی هستند.",
        tone: "warning",
      });
      return;
    }
  ...
  async function saveRescheduleDraft() {
    if (
      !rescheduleDraftPost ||
      !rescheduleDraftIso ||
      rescheduleDraftConflicts.length > 0
    )
      return;
    ...
    setPosts((current) =>
      current.map((item) =>
        item.id === rescheduleDraftPost.id
          ? { ...item, scheduled_at: nextScheduledAt }
          : item,
      ),
    );
    ...
    try {
      const response = await fetch(
        `${apiUrl}/posts/${rescheduleDraftPost.id}/schedule`,
        {
          method: "POST",
      ...
      setPosts((current) =>
        current.map((item) =>
          item.id === rescheduleDraftPost.id ? savedPost : item,
        ),
      );
      setRescheduleDraftPostId(null);
      showToast({
        title: "زمان انتشار بروزرسانی شد",
        description: `${savedPost.title} · ${formatJalaliDateTime(savedPost.scheduled_at)}`,
        tone: "success",
      });
      focusSelectedDayAgenda(nextDayKey);
    } catch (err) {
      const nextError =
        err instanceof Error ? err.message : "تغییر زمان پست ناموفق بود";
      setPosts((current) =>
        current.map((item) =>
          item.id === rescheduleDraftPost.id
            ? { ...item, scheduled_at: originalScheduledAt }
            : item,
        ),
      );
      ...
      setError(nextError);
      showToast({
        title: "تغییر زمان پست ناموفق بود",
  ...
  async function reschedulePost(post: Post, day: CalendarDay) {
    ...
    const scheduledAt = jalaliDateToIsoAtTime(
      day.date,
      time.hour,
      time.minute,
      scheduleTimezone,
    ...
    if (!scheduledAt) {
      showToast({
        title: "جابجایی پست ناموفق بود",
        description: "تاریخ مقصد قابل تبدیل نیست.",
        tone: "alert",
      });
      return;
    }
    ...
    setPosts((current) =>
      current.map((item) =>
        item.id === post.id ? { ...item, scheduled_at: scheduledAt } : item,
      ),
    );
    setSelectedDayKey(day.key);
    setSelectedPostId(post.id);
    ...
    try {
      ...
      const savedPost = (await response.json()) as Post;
      setPosts((current) =>
        current.map((item) => (item.id === post.id ? savedPost : item)),
      );
      showToast({
        title: "زمان انتشار جابجا شد",
        description: `${post.title} · ${formatJalaliDateTime(savedPost.scheduled_at)}`,
        tone: "success",
      });
    } catch (err) {
      const nextError =
        err instanceof Error ? err.message : "جابجایی پست ناموفق بود";
      setPosts((current) =>
        current.map((item) =>
          item.id === post.id
            ? { ...item, scheduled_at: originalScheduledAt }
  ...
  function dropPostOnDay(event: DragEvent<HTMLDivElement>, day: CalendarDay) {
    event.preventDefault();
    const postId = Number(
      event.dataTransfer.getData("text/plain") || draggingPostId,
    );
  ...
  function renderPostChip(post: Post, compact = false) {
    ...
    return (
      <button
        ...
        style={
          {
            "--campaign-accent": campaignColorForPost(post, campaigns),
          } as CSSProperties
        }
        title={
          draggable
            ? "برای تغییر روز انتشار، پست را روی روز جدید بکشید."
            : undefined
        }
      >
        ...
        <span className="calendar-post-chip-inner">
          {previewUrl ? (
            ...
            <span className="calendar-post-chip-media calendar-post-chip-media-empty">
              <ImageIcon
                className="calendar-post-chip-media-icon"
                aria-hidden="true"
              />
            </span>
          ) : null}
          <span className="calendar-post-chip-body">
            <span className="calendar-post-chip-top">
              <span className="calendar-post-chip-time">
                {formatJalaliTime(post.scheduled_at)}
              </span>
              <span className="calendar-post-chip-status">
            ...
            <span className="calendar-post-chip-campaign">
              ...
              <span className="truncate">
                {campaignKeyForPost(post) !== "none"
                  ...
                  : (!compact ? post.caption : "") || "بدون کمپین"}
              </span>
            </span>
          </span>
        </span>
      ...
      >
        ...
        {previewUrl ? (
          <img
            ...
            className="calendar-day-agenda-thumb"
          />
        ) : (
        ...
        <div className="calendar-day-agenda-body">
          <div className="calendar-day-agenda-meta">
            <span className="calendar-day-agenda-time">
              {formatJalaliTime(post.scheduled_at)}
            </span>
          ...
          <div className="calendar-day-agenda-tags">
            <span
              className="calendar-post-preview-campaign"
              style={
                {
                  "--campaign-accent": campaignColorForPost(post, campaigns),
                } as CSSProperties
        ...
        <div className="calendar-day-agenda-actions">
          ...
          <Button href={`/compose?postId=${post.id}`} variant="ghost" size="sm">
            ویرایش
          </Button>
        </div>
      </article>
  ...
  function renderMonthDayDensity(dayPosts: Post[], hiddenCount: number) {
    if (dayPosts.length === 0) {
      return (
        <span className="calendar-day-empty-hint">برای ساخت پست کلیک کنید</span>
      );
    }

    return (
      <div
        className="calendar-day-density-list"
        aria-label={`${dayPosts.length} پست در این روز`}
      >
        {dayPosts.slice(0, 3).map((post) => (
          <span
            key={post.id}
            className="calendar-day-density-item"
            style={
              {
                "--campaign-accent": campaignColorForPost(post, campaigns),
              } as CSSProperties
        ...
        {hiddenCount > 0 ? (
          <span className="calendar-day-density-more">
            +{hiddenCount} بیشتر
          </span>
        ) : null}
      </div>
      ...
      <div className="flex flex-col gap-4 overflow-y-auto max-h-[80vh] md:max-h-none p-1">
        <div className="calendar-post-preview-media">
          {previewUrl ? (
            <img
              src={previewUrl}
              alt={asset?.original_filename ?? ""}
              className="calendar-post-preview-image rounded-lg w-full max-h-48 object-cover border border-app-border"
            />
          ) : (
        ...
        <div className="calendar-post-preview-details calendar-post-detail-summary flex flex-col gap-2">
          <div className="calendar-post-preview-status flex flex-wrap gap-1.5 items-center">
            ...
            />
            <span
              className="calendar-post-preview-campaign text-xs font-black inline-flex items-center gap-1 px-2 py-0.5 rounded-full border border-app-border bg-white"
              style={
                {
                  "--campaign-accent": campaignColorForPost(post, campaigns),
                } as CSSProperties
            ...
            >
              <span
                className="h-2 w-2 rounded-full bg-[var(--campaign-accent)]"
                aria-hidden="true"
              />
              {campaignLabelForPost(post, campaigns)}
            </span>
          ...
          <DetailGrid
            items={[
              {
                label: "زمان انتشار",
                value: formatJalaliDateTime(post.scheduled_at),
              },
          ...
          <div className="calendar-post-readiness-grid grid grid-cols-2 gap-2 mt-2">
            {quickPreviewReadiness.map((item) => (
              <div
                key={item.label}
                className="calendar-post-readiness-item flex flex-col p-2 rounded-md border border-app-border bg-slate-50"
                data-tone={item.tone}
              >
                <span className="text-[10px] text-app-muted font-bold">
                  {item.label}
                </span>
                <strong className="text-xs font-black mt-0.5">
                  {item.value}
                </strong>
              </div>
            ))}
        ...
        <div className="calendar-post-detail-grid flex flex-col gap-3 mt-2">
          <section className="calendar-post-detail-card p-3 rounded-lg border border-app-border bg-slate-50">
            <p className="app-section-kicker text-[10px] font-black mb-1">
              کپشن
            </p>
            <p className="calendar-post-preview-caption text-xs leading-relaxed text-app-text whitespace-pre-wrap">
              {post.caption || "کپشن برای این پست ثبت نشده است."}
            </p>
          </section>
          <section className="calendar-post-detail-card p-3 rounded-lg border border-app-border bg-slate-50">
            <p className="app-section-kicker text-[10px] font-black mb-1.5">
              مسیر انتشار
            </p>
            <Timeline
              items={[
        ...
        {post.last_error ? (
          <NoticeBanner tone="alert">{post.last_error}</NoticeBanner>
        ) : null}
      </div>
    );
  }
  ...
  function renderPostPreviewFooter(post: Post) {
    return (
      <div className="flex flex-wrap gap-2 justify-end w-full border-t border-app-border p-3 bg-slate-50">
        <Button href={`/compose?postId=${post.id}`} size="sm">
          ویرایش پست
        </Button>
        {canReschedule(post) ? (
          <Button
            ...
            size="sm"
            onClick={() =>
              openRescheduleDraft(post, post.scheduled_at ?? selectedDayValue)
            }
          >
            تغییر زمان
          </Button>
        ) : null}
        <Button href="/queue" variant="secondary" size="sm">
          صف انتشار
        </Button>
        <Button
          type="button"
          variant="ghost"
          size="sm"
          onClick={() => setQuickPreviewPostId(null)}
        >
          بستن
        </Button>
      </div>
      ...
      <div className="flex flex-col gap-4 p-2 overflow-y-auto h-full">
        <div className="calendar-reschedule-body flex flex-col gap-3">
          <div className="calendar-reschedule-day-card p-3 rounded-lg border border-app-border bg-slate-50">
            ...
            <div className="calendar-reschedule-day-value flex items-center gap-2 mt-1.5 text-sm font-black text-app-text">
              ...
              <span>
                {rescheduleDraftDay
                  ? formatJalaliDate(rescheduleDraftDay)
                  : "روز انتخاب نشده"}
              </span>
            </div>
            <div className="calendar-reschedule-day-actions flex gap-2 mt-2.5">
              <Button
                ...
                variant="secondary"
                size="sm"
                onClick={() => setRescheduleDraftDay(selectedDayValue)}
              >
                روز فعال تقویم
              </Button>
              <Button
                ...
                size="sm"
                onClick={() =>
                  setRescheduleDraftDay(
                    addDays(
                      post.scheduled_at ?? selectedDayValue,
                      1,
                    ).toISOString(),
                  )
                }
          ...
          <div className="calendar-reschedule-time-card p-3 rounded-lg border border-app-border bg-slate-50">
            ...
            <div className="calendar-reschedule-selectors grid grid-cols-2 gap-3 mt-2">
              <label className="flex flex-col gap-1">
                ...
                <select
                  value={rescheduleDraftHour}
                  onChange={(event) =>
                    setRescheduleDraftHour(Number(event.target.value))
                  }
                  className="w-full border border-app-border bg-white rounded-md p-1.5 text-xs outline-none"
                >
              ...
              <label className="flex flex-col gap-1">
                ...
                <select
                  value={rescheduleDraftMinute}
                  onChange={(event) =>
                    setRescheduleDraftMinute(Number(event.target.value))
                  }
                  className="w-full border border-app-border bg-white rounded-md p-1.5 text-xs outline-none"
                >
            ...
            <div className="calendar-reschedule-slots flex flex-wrap gap-1.5 mt-3">
              {suggestedSlotsForDay(
                rescheduleDraftDay || selectedDayValue,
                post.id,
              )
                .slice(0, 5)
                .map((slot) => (
                  <Button
                    ...
                    type="button"
                    variant={
                      rescheduleDraftHour === slot.hour &&
                      rescheduleDraftMinute === slot.minute
                        ? "primary"
                        : "secondary"
                    }
                    size="sm"
        ...
        {rescheduleDraftConflicts.length ? (
          <NoticeBanner tone="alert" title="تداخل زمانی">
            این زمان با {rescheduleDraftConflicts.length} پست دیگر کمتر از ۹۰
            دقیقه فاصله دارد. یک ساعت پیشنهادی دیگر انتخاب کنید.
          </NoticeBanner>
        ) : (
  ...
  function renderRescheduleFooter(post: Post) {
    return (
      <div className="flex gap-2 justify-end w-full border-t border-app-border p-3 bg-slate-50">
        <Button
          type="button"
          onClick={() => void saveRescheduleDraft()}
          disabled={
            !rescheduleDraftIso ||
            rescheduleDraftConflicts.length > 0 ||
        ...
        <Button
          type="button"
          variant="secondary"
          onClick={() => setRescheduleDraftPostId(null)}
          size="sm"
        >
          انصراف
        </Button>
    ...
    <AuthGate>
      <AppShell>
        <WorkspacePage className="calendar-pro-page">
          <nav
            className="flex items-center gap-1 rounded-lg border border-app-border bg-app-surface/70 px-1.5 py-1.5 shadow-hairline backdrop-blur-sm"
            aria-label="زیرمنوی برنامه‌ریز"
          >
            <Link
              href="/calendar"
              className="inline-flex items-center gap-2 rounded-md px-3 py-1.5 text-sm font-black bg-white text-app-primary shadow-hairline border border-app-border/80 transition"
            >
            ...
            <Link
              href="/queue"
              className="inline-flex items-center gap-2 rounded-md px-3 py-1.5 text-sm font-bold text-app-muted hover:bg-white hover:text-app-text hover:shadow-hairline transition"
            >
          ...
          <section className="calendar-pro-hero">
            <div className="calendar-pro-hero-copy">
              <p className="app-section-kicker calendar-pro-kicker text-[10px] font-black">
                برنامه‌ریز
              </p>
              <h1>تقویم انتشار</h1>
              <p>
                نمای عملیاتی برنامه محتوا؛ ببینید چه چیزی زمان‌دار است، چه چیزی
                گیر کرده و کدام روز ظرفیت دارد.
              </p>
            </div>
            <div className="calendar-pro-hero-actions">
              <StatusToken tone={plannerHealthTone}>
                {plannerHealthLabel}
              </StatusToken>
              <Button
                type="button"
                onClick={() => openQuickCreate(selectedDayValue)}
                size="sm"
              >
          ...
          <section
            className="dashboard-kpi-strip calendar-kpi-strip grid grid-cols-2 gap-3 lg:grid-cols-4"
            aria-label="خلاصه تقویم انتشار"
          >
            {[
              {
                label: "بازه فعال",
                value: currentRangeLabel,
                detail: `${visibleRangeCount} در دید · ${publishedCount} منتشر`,
                tone: "primary" as const,
                icon: CalendarDays,
              },
              {
              ...
              },
              {
                ...
                detail: failedCount ? `${failedCount} ناموفق` : "بدون مانع جدی",
                tone: attentionPosts.length
                  ? ("alert" as const)
                  : ("success" as const),
                icon: AlertCircle,
              },
          ...
          {error ? (
            <NoticeBanner tone="alert" title="نیاز به بررسی">
              {error}
            </NoticeBanner>
          ) : null}
          ...
          <section className="calendar-pro-workspace grid xl:grid-cols-[minmax(0,1fr)_360px] gap-4">
            <section
              className={`app-studio-panel calendar-pro-planner min-w-0 overflow-hidden rounded-lg ${mobileView === "grid" ? "block" : "hidden xl:block"}`}
            >
                  ...
                  <div className="calendar-toolbar-title-row flex flex-wrap items-center gap-2">
                    ...
                    <div className="calendar-toolbar-status-chips flex flex-wrap items-center gap-2">
                      <StatusToken tone="neutral">
                        {monthPostCount} پست در ماه
                      </StatusToken>
                      {nextPost ? (
                        <StatusToken tone="success">
                          بعدی: {formatJalaliDateTime(nextPost.scheduled_at)}
                        </StatusToken>
                      ) : null}
                      <StatusToken
                        tone={attentionPosts.length ? "alert" : "success"}
                      >
                        {attentionPosts.length
                          ? `${attentionPosts.length} نیازمند توجه`
                          : "برنامه پایدار"}
                      </StatusToken>
                      <StatusToken tone="info">
                        پست زمان‌بندی‌شده را برای تغییر روز بکشید
                      </StatusToken>
                      {reschedulingPostId ? (
                        <StatusToken tone="warning">
                          در حال ذخیره جابجایی
                        </StatusToken>
                      ) : null}
                    </div>
                  </div>
                ...
                <div className="calendar-toolbar-filter-row mt-2 grid gap-2 2xl:grid-cols-[minmax(220px,1fr)_190px_auto_auto] 2xl:items-center">
                  ...
                  <label className="calendar-toolbar-campaign-filter flex items-center gap-2 rounded-md border border-app-border bg-white px-3 py-2 text-xs font-bold text-app-muted shadow-hairline">
                    ...
                    <select
                      ...
                      className="min-w-0 flex-1 bg-transparent text-xs font-bold text-app-text outline-none"
                    >
                  ...
                  <div
                    className="calendar-segment-control"
                  ...
                  <div
                    className="calendar-segment-control calendar-toolbar-density-toggle"
                    aria-label="تراکم تقویم"
                  >
                    <button
                      type="button"
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-app-border bg-white px-3 py-2 shadow-hairline">
                    <span className="inline-flex items-center gap-2 text-xs font-black text-app-text">
                      <span
                        ...
                        style={{
                          backgroundColor: selectedCampaignOption.color,
                        }}
                      />
                      برنامه کمپین: {selectedCampaignOption.label}
                    </span>
                    <StatusToken tone="neutral">
                      {selectedCampaignWorkload.total} در بازه
                    </StatusToken>
                    <StatusToken tone="warning">
                      {selectedCampaignWorkload.scheduled} زمان‌بندی
                    </StatusToken>
                    <StatusToken tone="primary">
                      {selectedCampaignWorkload.publishing} در انتشار
                    </StatusToken>
                    <StatusToken tone="success">
                      {selectedCampaignWorkload.published} منتشر
                    </StatusToken>
                    <StatusToken
                      tone={
                        selectedCampaignWorkload.failed ? "alert" : "success"
                      }
                    >
                      {selectedCampaignWorkload.failed} خطا
                    </StatusToken>
                    {selectedCampaignIdForRoute ? (
                      <Button
                        href={`/campaigns?campaignId=${selectedCampaignIdForRoute}`}
                        variant="ghost"
                        size="sm"
                      >
                        مدیر کمپین
                ...
                <div className="calendar-planner-viewport">
                  <div className="calendar-planner-board">
                    ...
                    <div className="calendar-days-grid grid grid-cols-7">
                      {(viewMode === "week" ? activeWeekDays : monthGrid).map(
                        (day, index) => {
                          const dayPosts = day
                            ? (postsByDay.get(day.key) ?? [])
                            : [];
                          const isToday = day?.key === todayKey;
                          const isSelectedDay = Boolean(
                            day && activeDayKey === day.key,
                          );
                          ...
                          const weekdayLabel = day
                            ? weekDays[weekOffset(new Date(day.date))]
                            : "";
                          const dayStateLabel = dayRisk
                            ? "ریسک"
                            : dayPosts.length
                              ? `${dayPosts.length} پست`
                              : "خالی";
                          ...
                          return (
                            <div
                              ...
                              onClick={() =>
                                day ? selectDay(day, dayPosts, true) : undefined
                              }
                              onDragOver={(event) =>
                                day ? allowDropOnDay(event, day) : undefined
                              }
                              onDrop={(event) =>
                                day ? dropPostOnDay(event, day) : undefined
                              }
                                ...
                                <>
                                  <div className="calendar-day-head">
                                    <span className="calendar-day-date">
                                      ...
                                      {hasConflict ? (
                                        <span
                                          ...
                                          title="تداخل فاصله انتشار کمتر از ۹۰ دقیقه"
                                        />
                                      ) : null}
                                    </span>
                                    <div className="calendar-day-actions">
                                      ...
                                      >
                                        <Plus
                                          className="calendar-day-add-icon"
                                          aria-hidden="true"
                                        />
                                      </button>
                                    </div>
                                  ...
                                  >
                                    {viewMode === "month" ? (
                                      renderMonthDayDensity(
                                        dayPosts,
                                        Math.max(0, dayPosts.length - 3),
                                      )
                                    ) : (
                                      <>
                                        {dayPosts
                                          .slice(0, visiblePostLimit)
                                          .map((post) =>
                                            renderPostChip(post, false),
                                          )}
                  ...
                  <div className="divide-y divide-app-border">
                    {filteredPosts.map((post) => {
                      ...
                      return (
                        <article
                          key={post.id}
                          className="relative grid gap-3 overflow-hidden px-4 py-3 transition hover:bg-slate-50 lg:grid-cols-[150px_minmax(0,1fr)_140px_110px] lg:items-center"
                        >
                          <span
                            className="absolute inset-y-0 right-0 w-1"
                            style={{
                              backgroundColor: campaignColorForPost(
                                post,
                                campaigns,
                              ),
                            }}
                          ...
                          <div className="text-xs leading-6 text-app-muted">
                            <p className="font-bold text-app-text">
                              {formatJalaliDate(post.scheduled_at)}
                            </p>
                          ...
                          <div className="flex min-w-0 items-center gap-3">
                            {previewUrl ? (
                              <img
                                src={previewUrl}
                                alt=""
                                className="h-10 w-10 shrink-0 rounded-md object-cover ring-1 ring-app-border"
                              />
                            ) : (
            ...
            <aside
              ref={agendaRef}
              className={`calendar-action-rail overflow-y-auto max-h-[85vh] xl:max-h-none ${agendaPulseKey ? "calendar-day-agenda-pulse" : ""} ${mobileView === "agenda" ? "block" : "hidden xl:block"}`}
              aria-label="عملیات برنامه‌ریز"
            >
              <div className="calendar-action-rail-head">
                <div className="min-w-0">
                  <p className="app-section-kicker text-[10px] font-black">
                    مرکز کنترل
                  </p>
                  <h2>برنامه روز {selectedDayLabel}</h2>
                  <p>
                    {selectedDayPosts.length
                      ? `${selectedDayPosts.length} پست زمان‌دار برای این روز`
                      : "روز آزاد برای ساخت برنامه جدید"}
                  </p>
                </div>
              ...
              {/* Next Action Banner */}
              <div
                className="calendar-day-next-action mt-3"
                data-tone={selectedDayNextAction.tone}
              >
                <span
                  className="calendar-day-next-action-icon"
                  aria-hidden="true"
                >
                  {selectedDayNextAction.tone === "alert" ? (
                    <AlertCircle className="h-4 w-4" />
                  ) : selectedDayNextAction.tone === "success" ? (
                ...
                <span className="min-w-0">
                  <strong className="block text-xs font-black">
                    {selectedDayNextAction.title}
                  </strong>
                  <small className="block text-[10px] text-app-muted mt-0.5">
                    {selectedDayNextAction.detail}
                  </small>
                </span>
              </div>
              ...
              {/* Slot Timeline */}
              <div
                className="calendar-day-slot-studio mt-3"
                aria-label="مسیر زمانی روز انتخاب‌شده"
              >
                <div className="calendar-day-slot-studio-head flex justify-between items-center text-xs mb-1.5">
                  ...
                  <span className="text-[10px] text-app-muted">
                    {selectedDayTimeline.length
                      ? `${selectedDayTimeline.length} نقطه زمانی`
                      : "بدون برنامه"}
                  </span>
                </div>
                  ...
                  {selectedDayTimeline.length ? (
                    selectedDayTimeline.map((item) =>
                      item.type === "post" ? (
                        ...
                        >
                          <span className="text-app-muted block font-semibold">
                            {item.label}
                          </span>
                          <strong className="text-app-text block truncate font-black mt-0.5">
                            {item.post.title}
                          </strong>
                        </button>
                      ) : (
                        <button
                          ...
                          onClick={() =>
                            openQuickCreateAt(selectedDayValue, item.hour, 0)
                          }
                        >
                          <span className="text-app-muted block font-semibold">
                            {item.label}
                          </span>
                          <strong className="text-app-primary block font-black mt-0.5">
                            زمان خالی
                          </strong>
                        </button>
                      ),
                    ...
                    <button
                      type="button"
                      className="calendar-day-slot-pill calendar-day-slot-pill-free flex-shrink-0 text-right p-1.5 rounded border border-dashed border-app-border text-[10px] bg-slate-50 w-full"
                      onClick={() => openQuickCreate(selectedDayValue)}
                    >
                      <span className="text-app-muted block">۰۹:۰۰</span>
                      <strong className="text-app-primary block font-black">
                        شروع برنامه
                      </strong>
                    </button>
                  )}
              ...
              <div className="calendar-day-agenda-list mt-3 flex flex-col gap-2">
                {selectedDayPosts.length ? (
                  selectedDayPosts.map((post) => renderAgendaPost(post))
                ) : (
                  <EmptyState
                    icon={
                      <CalendarDays className="h-5 w-5" aria-hidden="true" />
              ...
              <div className="calendar-action-rail-stack flex flex-col gap-2 mt-4 pt-3 border-t border-app-border">
                <Button
                  type="button"
                  onClick={() => openQuickCreate(selectedDayValue)}
                  className="calendar-action-primary w-full justify-center"
                  size="sm"
                >
                  <Plus className="ml-1.5 h-4 w-4" aria-hidden="true" />
                ...
                {selectedPost ? (
                  <>
                    <Button
                      ...
                      onClick={() => setQuickPreviewPostId(selectedPost.id)}
                    >
                    ...
                    {canReschedule(selectedPost) ? (
                      <Button
                        ...
                        className="w-full justify-center"
                        onClick={() =>
                          openRescheduleDraft(selectedPost, selectedDayValue)
                        }
                      >
                        تغییر زمان پست
                      </Button>
                    ...
                    <Button
                      href={`/compose?postId=${selectedPost.id}`}
                      variant="secondary"
                      className="w-full justify-center"
                      size="sm"
                    >
                ...
                <div className="grid grid-cols-2 gap-2 mt-1">
                  <Button
                    href="/queue"
                    variant="ghost"
                    className="w-full justify-center shadow-hairline border border-app-border/80 text-xs"
                    size="sm"
                  >
                    صف انتشار
                  </Button>
              ...
              <div className="calendar-action-rail-status mt-3 pt-3 border-t border-app-border text-center">
                {attentionPosts.length ? (
                  <Link
                    ...
                    className="calendar-action-rail-alert inline-flex w-full justify-center"
                  >
                    <AlertCircle
          ...
          {quickPreviewPost ? (
            isMobile ? (
              ...
              createPortal(
                <div
                  className="calendar-post-preview-backdrop"
                  role="presentation"
                  onClick={() => setQuickPreviewPostId(null)}
                >
                  <section
                    className="calendar-post-preview-modal calendar-post-detail-sheet animate-nashville-in"
                    role="dialog"
                    aria-modal="true"
                    aria-label={`جزئیات ${quickPreviewPost.title}`}
                  ...
                  >
                    <div className="calendar-post-preview-head border-b border-app-border pb-3 mb-3">
                      <div className="min-w-0">
                        ...
                        <h2 className="text-sm font-black text-app-text mt-1">
                          {quickPreviewPost.title}
                        </h2>
                        <p className="text-[10px] text-app-muted mt-0.5">
                          {formatJalaliDateTime(quickPreviewPost.scheduled_at)}{" "}
                          · {campaignLabelForPost(quickPreviewPost, campaigns)}
                        </p>
                      </div>
                      <button
                        type="button"
                        className="calendar-post-preview-close"
                        onClick={() => setQuickPreviewPostId(null)}
                        aria-label="بستن پیش‌نمایش"
                      >
                        <X
                          className="calendar-post-preview-close-icon"
                          aria-hidden="true"
                        />
                      </button>
                    </div>
          ...
          {rescheduleDraftPost ? (
            isMobile ? (
              ...
              createPortal(
                ...
                >
                  <section
                    className="calendar-reschedule-modal animate-nashville-in"
                    role="dialog"
                    aria-modal="true"
                    aria-label={`تغییر زمان ${rescheduleDraftPost.title}`}
                    onClick={(event) => event.stopPropagation()}
                  >
                    <div className="calendar-post-preview-head border-b border-app-border pb-3 mb-3">
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-app-muted mt-0.5">
                          ...
                          · زمان فعلی:{" "}
                          {formatJalaliDateTime(
                            rescheduleDraftPost.scheduled_at,
                          )}
                        </p>
                      </div>
                      <button
          ...
          <PlannerComposerDrawer
            scheduledAt={quickCreateAt}
            defaultCampaign={selectedCampaignOption?.label ?? ""}
