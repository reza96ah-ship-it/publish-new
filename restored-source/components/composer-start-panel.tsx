    ...
    <section className="app-studio-panel overflow-hidden rounded-lg">
      <div className="grid lg:grid-cols-[minmax(0,1fr)_240px]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="mt-1 text-lg font-black text-app-text">
                پست را از مسیر مناسب شروع کنید
              </h2>
        ...
        <div className="border-t border-app-border bg-app-canvas/70 p-4 lg:border-r lg:border-t-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-[10px] font-bold text-app-muted">
              {mediaPreviewUrls.length
                ? `${mediaPreviewUrls.length} پیش‌نمایش`
                : "کتابخانه خالی"}
            </span>
          </div>
          {mediaPreviewUrls.length ? (
            <div className="mt-3 grid grid-cols-3 gap-1.5">
              {mediaPreviewUrls.map((previewUrl, index) => (
                <img
                  key={previewUrl}
                  src={previewUrl}
                  alt={`رسانه آماده ${index + 1}`}
                  className="aspect-square w-full rounded-md object-cover shadow-hairline"
                />
              ))}
            </div>
          ) : (
          ...
          <p className="mt-3 text-[11px] leading-5 text-app-muted">
            تصویرها از کتابخانه همین فضای کاری نمایش داده می‌شوند.
          </p>
        </div>
      </div>
    </section>
