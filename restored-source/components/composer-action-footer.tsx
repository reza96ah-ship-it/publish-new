...
export function ComposerActionFooter({
  ...
  const primaryAction = hasSchedule
    ? "schedule"
    : canMarkReady
      ? "ready"
      : "draft";
  ...
  return (
    <div className="sticky bottom-4 z-20 mt-4 overflow-hidden rounded-lg border border-white/70 bg-white/85 px-3 py-2.5 shadow-lift ring-1 ring-app-border/80 backdrop-blur-xl">
      <div
        className="absolute inset-x-3 top-0 h-px bg-gradient-to-r from-transparent via-app-primary/25 to-transparent"
        aria-hidden="true"
      />
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div className="min-w-0">
          <p className="text-sm font-black text-app-text">
            {hasSchedule
              ? "آماده بررسی و ورود به صف"
              : canMarkReady
                ? "محتوا آماده است؛ زمان‌بندی اختیاری است"
                : "پست را مرحله‌به‌مرحله کامل کنید"}
          </p>
          <p className="mt-1 text-xs leading-5 text-app-muted">
            {hasSchedule
              ? "پیش‌نمایش و کنترل‌های نهایی را بررسی کنید."
              : canMarkReady
                ? "می‌توانید پست را آماده کنید یا از بازرس انتشار زمان انتخاب کنید."
                : "عنوان، متن یا رسانه، و کانال انتشار وضعیت دکمه اصلی را تعیین می‌کنند."}
          </p>
          {autosaveLabel ? (
