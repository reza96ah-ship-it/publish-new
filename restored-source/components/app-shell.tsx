...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(workspaceUpdatedEvent, refreshOverview);
  }, []);
  ...
  const brandAssetId =
    overview.store?.avatar_asset_id ?? overview.store?.logo_asset_id ?? null;
  ...
  return (
    <main className="app-workspace-bg h-screen overflow-hidden text-app-text">
      <div
        ...
        className="flex h-full min-h-0 overflow-y-auto overscroll-contain scroll-smooth"
      >
        <Sidebar
        ...
        <section className="nahrino-shell flex min-h-full min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-20 shrink-0 bg-white/70 backdrop-blur-2xl border-b border-slate-200/60 supports-[backdrop-filter]:bg-white/40">
            <div className="flex min-h-[64px] items-center justify-between gap-3 px-4 lg:px-8">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <span
                  className="hidden h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-app-primary/10 to-app-primary/5 text-app-primary shadow-sm ring-1 ring-app-primary/20 lg:flex"
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 truncate text-[15px] font-black tracking-tight text-slate-900">
                    {activeNav.item.label}
                  </p>
                </div>
              </div>
              ...
              <div className="flex min-w-0 items-center gap-2.5">
                ...
                >
                  ...
                  <span className="truncate font-medium">
                    جست‌وجوی پست، مسیر یا کمپین...
                  </span>
                  <span className="mr-auto hidden rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-[10px] font-bold text-slate-400 xl:inline shadow-sm">
                    Ctrl K
                  </span>
                </button>
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-2 right-2 h-2 w-2 rounded-full ring-2 ring-white ${liveNotificationsReady ? "bg-emerald-500" : "bg-slate-300"}`}
                    aria-label={
                ...
                <div className="relative">
                  ...
                  >
                    <WorkspaceAvatar
                      ...
                      className="h-7 w-7 rounded-lg ring-2 ring-white shadow-sm"
                    />
                    ...
                    <div className="absolute left-0 top-[48px] w-[280px] overflow-hidden rounded-2xl border border-slate-200/60 bg-white/95 backdrop-blur-2xl shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)] ring-1 ring-black/5 animate-in fade-in slide-in-from-top-2 duration-200 z-50">
                      <div className="border-b border-slate-100 px-4 py-4 bg-slate-50/50">
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <div className="min-w-0">
                            <p className="flex items-center gap-1.5 text-[13px] font-black text-slate-900">
                              <Sparkles
                      ...
                      <div className="p-2">
                        <Link
                          href="/store"
                          className="flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-[13px] font-bold text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900"
                        >
                          <Settings2
                            className="h-4 w-4 text-slate-400"
                            aria-hidden="true"
          ...
          <div className="app-enter relative px-3 py-3 pb-24 sm:px-4 lg:px-5 lg:py-4">
            {children}
          </div>
          <MobileNav />
          <CommandPalette
